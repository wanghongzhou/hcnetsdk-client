...
public interface HCNetSDK extends Library {
    ...
    HCNetSDK INSTANCE = Native.load(Platform.isWindows() ? "HCNetSDK" : "libhcnetsdk", HCNetSDK.class);
    ...
    int MAX_IP_ALARMIN_V40 = 4096;    //允许加入的最多报警输入数
    int MAX_IP_ALARMOUT_V40 = 4096;    //允许加入的最多报警输出数
    int MAX_ALARMOUT_V40 = (MAX_IP_ALARMOUT_V40 + MAX_ANALOG_ALARMOUT); //4128
    int MAX_ALARMIN_V40 = (MAX_IP_ALARMIN_V40 + MAX_ANALOG_ALARMOUT); //4128
    ...
    /* 最大支持的通道数 最大模拟加上最大IP支持 */ int MAX_CHANNUM_V30 = (MAX_ANALOG_CHANNUM + MAX_IP_CHANNEL);//64
    ...
    int MAX_LICENSE_LEN_EX = 32; //车牌号最大长度
    int MAX_CARDNO_LEN = 48;     //卡号最大长度
    ...
    int MAX_CARD_READER_NUM_512 = 512;  //最大读卡器数
    int ERROR_MSG_LEN = 32;   //下发错误信息
    int MAX_FACE_NUM = 2;    //最大人脸数
    int MAX_FINGER_PRINT_LEN = 768;  //最大指纹长度

    int DEV_TYPE_NAME_LEN = 24;      //设备类型名称长度
    int MAX_FACE_PIC_NUM = 30;      /*人脸子图个数*/
    int CARDNUM_LEN_V30 = 40;

    int MAX_NOTICE_NUMBER_LEN = 32;   //公告编号最大长度
    int MAX_NOTICE_THEME_LEN = 64;   //公告主题最大长度
    int MAX_NOTICE_DETAIL_LEN = 1024; //公告详情最大长度
    int MAX_NOTICE_PIC_NUM = 6;    //公告信息最大图片数量
    int MAX_DEV_NUMBER_LEN = 32;   //设备编号最大长度
    int LOCK_NAME_LEN = 32;  //锁名称

    int NET_SDK_EMPLOYEE_NO_LEN = 32;  //工号长度
    int NET_SDK_UUID_LEN = 36;  //UUID长度

    int MAX_INQUEST_CDRW_NUM = 4;     //最大刻录机数目
    int INQUEST_MESSAGE_LEN = 44;    //审讯重点标记信息长度
    int INQUEST_MAX_ROOM_NUM = 2;     //最大审讯室个数
    int MAX_RESUME_SEGMENT = 2;     //支持同时恢复的片段数目

    int SUPPORT_PD_NUM = 16;
    int SUPPORT_ARRAY_NUM = 8;
    int SUPPORT_VD_NUM = 128;
    int SUPPORT_PD_NUM_ = 16;
    int SUPPORT_PD_NUM_PARTTWO = 8;

    int CARDNUM_LEN_OUT = 32; //外部结构体卡号长度
    int GUID_LEN = 16;        //GUID长度

    int MAX_ROIDETECT_NUM = 8;    //支持的ROI区域数
    int MAX_LANERECT_NUM = 5;    //最大车牌识别区域数
    int MAX_FORTIFY_NUM = 10;   //最大布防个数
    int MAX_INTERVAL_NUM = 4;    //最大时间间隔个数
    int MAX_CHJC_NUM = 3;    //最大车辆省份简称字符个数
    int MAX_VL_NUM = 5;    //最大虚拟线圈个数
    int MAX_DRIVECHAN_NUM = 16;  //最大车道数
    int MAX_COIL_NUM = 3;    //最大线圈个数
    int MAX_SIGNALLIGHT_NUM = 6;   //最大信号灯个数
    int MAX_IOSPEED_GROUP_NUM = 4; //IO测速组个数
    int MAX_IOOUT_NUM = 4;  //最大IO输出口个数
    int MAX_IOIN_NUM = 8;  //最大IO输入口个数
    int MAX_RELAY_NUM = 12; //继电器控制设备最大数 2013-11-04
    int MAX_VEHICLE_TYPE_NUM = 8;  //车辆信息管控最大数2013-11-04
    int MAX_IOIN_NUMEX = 10; //最大IO输入口个数(扩展)
    int MAX_ITC_LANE_NUM = 6;  //最大车道个数
    int MAX_LANEAREA_NUM = 2;  //单车道最大区域个数
    int ITC_MAX_POLYGON_POINT_NUM = 20;    //检测区域最多支持20个点的多边形
    int MAX_ITC_SERIALCHECK_NUM = 8; //串口校验类型个数
    int MAX_LIGHT_NUM = 6; //最大交通灯数
    int MAX_VIDEO_INTERVAL_NUM = 2;  //最大抓拍间隔数
    int MAX_VIDEO_DETECT_LIGHT_NUM = 12;  //视频检测的最大检测区域
    int MAX_CALIB_RECOG_NUM = 2;  //标定区域个数
    int MAX_RS485_NUM = 12; //485口最大支持数
    int MAX_MOBILE_POLYGON_NUM = 3; //移动布控支持最大牌识区域个数
    int MAX_MOBILE_DETECTLINE_NUM = 3; //移动布控支持最大违规检测线个数
    int MAX_IOOUT_K_NUM = 8;  //K系列最大IO输出口个数

    int NET_SDK_MAX_FDID_LEN = 256; //人脸库ID最大长度
    int NET_SDK_MAX_PICID_LEN = 256; //人脸ID最大长度
    int NET_SDK_MAX_INDENTITY_KEY_LEN = 64; //交互操作口令长度

    int SEARCH_EVENT_INFO_LEN = 300;    //事件信息长度
    int SEARCH_EVENT_INFO_LEN_V40 = 800;
    int MAX_POS_KEYWORDS_NUM = 3; //支持关键字查找条数
    int MAX_POS_KEYWORD_LEN = 128; //每条关键字长度
    int INQUEST_CASE_LEN = 64;    //审讯信息长度

    int SEARCH_CASE_NO_LEN = 56;
    int SEARCH_CASE_NAME_LEN = 100;
    int SEARCH_LITIGANT_LEN = 32;
    int SEARCH_CHIEF_JUDGE_LEN = 32;
    int CASE_NO_RET_LEN = 52;
    int CASE_NAME_RET_LEN = 64;
    int LITIGANT_RET_LEN = 24;
    int CHIEF_JUDGE_RET_LEN = 24;
    int NET_SDK_CASETYPE_LEN = 32;
    int NET_SDK_MAX_TAPE_INDEX_LEN = 32;  //磁带编号最大长度
    int NET_SDK_MAX_FILE_LEN = 256;  //文件名最大长度
    /******************************************************************/
    ...
    int NET_DVR_SET_TRANS_TYPE = 32; //设置转码格式
    ...
    int NET_DVR_SET_DEVICECFG_V40 = 1101; //设置扩展设备参数
    ...
    int NET_DVR_COMPLETE_RESTORE_CTRL = 3420;    //设置完全恢复出厂值
    /***************************
    ...
    int NET_DVR_SET_PICCFG_V40 = 6180;   //设置图象参数
    int NET_DVR_GET_AES_KEY = 6113;    //获取设备AES加密密钥
    ...
    int NET_DVR_SET_IPPARACFG_V40 = 1063;    //设置IP接入配置信息
    ...
    int NET_SDK_FINDMEDICALFILE = 3954;    //慧影科技智慧医疗查找录像文件
    int NET_SDK_FINDMEDICALPICTURE = 3955;    //慧影科技智慧医疗查找图片文件

    int NET_DVR_GET_RAPIDMOVE_DETECTION = 3539;    //获取快速运动侦测配置
    int NET_DVR_SET_RAPIDMOVE_DETECTION = 3540;    //设置快速运动侦测配置

    int NET_DVR_GET_RAPIDMOVE_TRIGGER = 3543;    //获取快速运动联动配置
    int NET_DVR_SET_RAPIDMOVE_TRIGGER = 3544;    //设置快速运动联动配置
    int NET_DVR_GET_RAPIDMOVE_SCHEDULE = 3545;    //获取快速运动的布防时间配置
    int NET_DVR_SET_RAPIDMOVE_SCHEDULE = 3546;    //设置快速运动的布防时间配置

    int NET_DVR_GET_PRESET_NAME = 3383; //获取预置点名称
    int NET_DVR_SET_PRESET_NAME = 3382; //设置预置点名称
    int NET_DVR_GET_RULECFG_V42 = 5049;  //获取行为分析参数(支持16条规则扩展)
    int NET_DVR_SET_RULECFG_V42 = 5050;  //设置行为分析参数(支持16条规则扩展)
    ...
    int NET_DVR_SET_TRAVERSE_PLANE_DETECTION = 3361;
    int NET_DVR_GET_FIELD_DETECTION = 3362; //获取区域侦测配置
    int NET_DVR_SET_FIELD_DETECTION = 3363; //设置区域侦测配置

    int NET_DVR_GET_STREAM_INFO = 6023; //获取已添加流ID信息
    int NET_DVR_GET_STREAM_RECORD_STATUS = 6021; //获取流状态信息

    int NET_DVR_GET_ALL_VEHICLE_CONTROL_LIST = 3124; //获取所有车辆禁止和允许名单信息
    int NET_DVR_VEHICLELIST_CTRL_START = 3133; //设置车辆禁止和允许名单信息(批量)
    int ENUM_SENDDATA = 0x0; //发送数据

    int NET_DVR_GET_LEDDISPLAY_CFG = 3673;
    int NET_DVR_SET_LEDDISPLAY_CFG = 3672;
    int NET_DVR_SET_VOICEBROADCAST_CFG = 3675;
    int NET_DVR_SET_CHARGE_ACCOUNTINFO = 3662;

    int NET_DVR_GET_TRAFFIC_DATA = 3141;   //长连接获取交通数据
    int NET_DVR_GET_TRAFFIC_FLOW = 3142;   //长连接获取交通流量
    ...
    int NET_DVR_GET_SUPPLEMENTLIGHT = 3728;  //获取内置补光灯配置协议
    int NET_DVR_SET_SUPPLEMENTLIGHT = 3729;  //设置内置补光灯配置协议

    int NET_DVR_GET_FACECONTRAST_TRIGGER = 3965;//获取人脸比对联动配置
    int NET_DVR_SET_FACECONTRAST_TRIGGER = 3966;//设置人脸比对联动配置

    int NET_DVR_GET_FACECONTRAST_SCHEDULE = 3968;//获取人脸比对布防时间配置
    int NET_DVR_SET_FACECONTRAST_SCHEDULE = 3969;//设置人脸比对布防时间配置

    int NET_DVR_INQUEST_GET_CDW_STATUS = 6350;  //获取审讯机刻录状态-长连接

    int NET_DVR_GET_REALTIME_THERMOMETRY = 3629; //实时温度检测
    int NET_DVR_GET_MANUALTHERM_INFO = 6706;     //手动测温实时获取
    int NET_DVR_GET_THERMOMETRY_MODE = 6765;//获取测温模式参数
    int NET_DVR_SET_THERMOMETRY_MODE = 6766;//设置测温模式参数
    int NET_DVR_GET_PTZABSOLUTEEX = 6696;
    int NET_DVR_GET_THERMOMETRY_PRESETINFO = 3624;  //获取测温预置点关联配置参数
    int NET_DVR_SET_THERMOMETRY_PRESETINFO = 3625;  //设置测温预置点关联配置参数
    int NET_DVR_GET_THERMOMETRYRULE_TEMPERATURE_INFO = 23001;//手动获取测温规则温度信息
    int NET_DVR_SET_DEVSERVER_CFG = 3258;//设置模块服务配置

    int NET_DVR_GET_PHY_DISK_INFO = 6306;   //获取物理磁盘信息
    int NET_DVR_GET_WORK_STATUS = 6189;   //获取设备工作状态
    int NET_DVR_GET_MONTHLY_RECORD_DISTRIBUTION = 6164; //获取月历录像分布

    int NET_DVR_GET_CURTRIGGERMODE = 3130;   //获取设备当前触发模式
    int NET_ITC_GET_TRIGGERCFG = 3003;  //获取触发参数
    int NET_ITC_SET_TRIGGERCFG = 3004;  //设置触发参数
    int NET_ITC_GET_VIDEO_TRIGGERCFG = 3017;  //获取视频电警触发参数
    int NET_ITC_SET_VIDEO_TRIGGERCFG = 3018;  //设置视频电警触发参数

    int NET_DVR_GET_MULTI_STREAM_COMPRESSIONCFG = 3216;//远程获取多码流压缩参数
    int NET_DVR_SET_MULTI_STREAM_COMPRESSIONCFG = 3217;//远程设置多码流压缩参数

    int NET_DVR_GET_CMS_CFG = 2070;
    int NET_DVR_SET_CMS_CFG = 2071;

    int NET_DVR_GET_ALARM_INFO = 4193;  //获取报警事件数据
    /***************************DS9000新增命令(_V30) end *****************************/
    ...
    int NET_DVR_GET_CARD_CFG_V50 = 2178;    //获取新卡参数(V50)
    int NET_DVR_SET_CARD_CFG_V50 = 2179;    //设置新卡参数(V50)
    int NET_DVR_GET_FACE_PARAM_CFG = 2507;    //获取人脸参数
    int NET_DVR_SET_FACE_PARAM_CFG = 2508;    //设置人脸参数
    int NET_DVR_DEL_FACE_PARAM_CFG = 2509;    //删除人脸参数
    int NET_DVR_CLEAR_ACS_PARAM = 2118; //清空门禁主机参数
    int NET_DVR_DEL_FINGERPRINT_CFG = 2152; //删除指纹参数
    int NET_DVR_GET_FINGERPRINT_CFG_V50 = 2183;    //获取指纹参数V50
    int NET_DVR_SET_FINGERPRINT_CFG_V50 = 2184;   //设置指纹参数V50
    int NET_DVR_DEL_FINGERPRINT_CFG_V50 = 2517;    //删除指纹参数V50
    int NET_DVR_GET_CARD_RIGHT_WEEK_PLAN_V50 = 2304;  //获取卡权限周计划参数V50
    int NET_DVR_SET_CARD_RIGHT_WEEK_PLAN_V50 = 2305;  //设置卡权限周计划参数V50
    int NET_DVR_GET_CARD_RIGHT_PLAN_TEMPLATE_V50 = 2322; //获取卡权限计划模板参数V50
    int NET_DVR_SET_CARD_RIGHT_PLAN_TEMPLATE_V50 = 2323;//设置卡权限计划模板参数V50
    int NET_DVR_SET_DOOR_STATUS_PLAN_TEMPLATE = 2107; //设置门状态计划模板参数
    int NET_DVR_GET_DOOR_CFG = 2108;    //获取门参数
    int NET_DVR_SET_DOOR_CFG = 2109;    //设置门参数
    int NET_DVR_GET_DOOR_STATUS_PLAN = 2110; //获取门状态计划参数
    int NET_DVR_SET_DOOR_STATUS_PLAN = 2111; //设置门状态计划参数
    int NET_DVR_GET_WEEK_PLAN_CFG = 2100; //获取门状态周计划参数
    int NET_DVR_SET_WEEK_PLAN_CFG = 2101; //设置门状态周计划参数
    int NET_DVR_GET_EVENT_CARD_LINKAGE_CFG_V50 = 2181;    //获取事件卡号联动配置参数(V50)
    int NET_DVR_SET_EVENT_CARD_LINKAGE_CFG_V50 = 2182;    //设置事件卡号联动配置参数(V50)
    int NET_DVR_CAPTURE_FACE_INFO = 2510;    //采集人脸信息
    int NET_DVR_CAPTURE_FINGERPRINT_INFO = 2504; //采集指纹信息
    int NET_DVR_GET_ACS_EVENT = 2514;//设备事件获取
    int NET_DVR_GET_CARD_READER_CFG_V50 = 2505;    //获取读卡器参数(V50)
    int NET_DVR_SET_CARD_READER_CFG_V50 = 2506;   //设置读卡器参数(V50)
    int NET_DVR_GET_REGISTER_INFO = 2511;    //登记信息获取
    int NET_DVR_GET_SMSRELATIVEPARA_V50 = 2512;   //获取短信相关参数
    int NET_DVR_SET_SMSRELATIVEPARA_V50 = 2513;    //设置短信相关参数
    int NET_DVR_GET_MULTI_CARD_CFG_V50 = 2515;    //获取多重卡参数V50
    int NET_DVR_SET_MULTI_CARD_CFG_V50 = 2516;    //设置多重卡参数V5
    int NET_DVR_GET_EVENT_CARD_LINKAGE_CFG_V51 = 2518;    //获取事件卡号联动配置参数(V51)
    int NET_DVR_SET_EVENT_CARD_LINKAGE_CFG_V51 = 2519;    //设置事件卡号联动配置参数(V51)

    int NET_DVR_SET_EXAM_INFO = 2530;    //考试信息下发
    int NET_DVR_SET_EXAMINEE_INFO = 2531;    //考生信息下发
    int NET_DVR_SEARCH_EXAM_COMPARE_RESULT = 2532;    //考试比对结果查询
    int NET_DVR_BULK_CHECK_FACE_PICTURE = 2533;    //批量校验人脸图片
    int NET_DVR_JSON_CONFIG = 2550;    //JSON透传数据
    int NET_DVR_FACE_DATA_RECORD = 2551;    //添加人脸数据到人脸库
    int NET_DVR_FACE_DATA_SEARCH = 2552;    //查询人脸库中的人脸数据
    int NET_DVR_FACE_DATA_MODIFY = 2553;    //修改人脸库中的人脸数据
    int NET_DVR_CAPTURE_DATA_SEARCH = 2554;    //查询离线采集数据集中数据
    int NET_DVR_SET_FORM_DATA = 2555;    //长连接设置表单数据
    int NET_DVR_GET_FORM_DATA = 2556;    //长连接获取表单数据
    ...
    int NET_DVR_GET_FINGERPRINT = 2563;
    int NET_DVR_SET_FINGERPRINT = 2564; //下发指纹
    int NET_DVR_DEL_FINGERPRINT = 2565;  //删除指纹
    int NET_DVR_GET_ACS_WORK_STATUS_V50 = 2180; //获取门禁主机工作状态
    int NET_DVR_GET_ACS_CFG = 2159; //获取门禁主机参数
    int NET_DVR_SET_ACS_CFG = 2160; //设置门禁主机参数
    int NET_DVR_BULK_UPLOAD_ID_BLOCKLIST = 2521;        //批量上传身份证禁止名单

    /*************************************人脸识别门禁一体机1.0 end**************************************/
    int NET_DVR_SET_SENSOR_CFG = 1180;//设置模拟量参数
    int NET_DVR_GET_SENSOR_CFG = 1181;//获取模拟量参数
    int NET_DVR_SET_ALARMIN_PARAM = 1182;//设置报警输入参数
    int NET_DVR_GET_ALARMIN_PARAM = 1183;
    //获取报警输入参数
    int NET_DVR_SET_ALARMOUT_PARAM = 1184;//设置报警输出参数
    int NET_DVR_GET_ALARMOUT_PARAM = 1185;//获取报警输出参数
    int NET_DVR_SET_SIREN_PARAM = 1186;//设置警号参数
    int NET_DVR_GET_SIREN_PARAM = 1187;//获取警号参数
    int NET_DVR_SET_ALARM_RS485CFG = 1188;//设置报警主机485参数
    int NET_DVR_GET_ALARM_RS485CFG = 1189;//获取报警主机485参数
    int NET_DVR_GET_ALARMHOST_MAIN_STATUS = 1190;//获取报警主机主要状态
    int NET_DVR_GET_ALARMHOST_OTHER_STATUS = 1191;//获取报警主机其他状态
    int NET_DVR_SET_ALARMHOST_ENABLECFG = 1192;//获取报警主机使能状态
    int NET_DVR_GET_ALARMHOST_ENABLECFG = 1193;//设置报警主机使能状态
    int NET_DVR_SET_ALARM_CAMCFG = 1194;//设置视频综合平台报警触发CAM操作配置
    int NET_DVR_GET_ALARM_CAMCFG = 1195;//设置视频综合平台报警触发CAM操作配置
    int NET_DVR_SET_ALARMHOST_RS485_SLOT_CFG = 2055;// 设置报警主机485槽位参数
    int NET_DVR_GET_ALARMHOST_RS485_SLOT_CFG = 2056;// 获取报警主机485槽位参数
    int NET_DVR_SET_VIDEOWALLDISPLAYMODE = 1730;//设置电视墙拼接模式
    int NET_DVR_GET_VIDEOWALLDISPLAYMODE = 1731;//获取电视墙拼接模式
    int NET_DVR_GET_VIDEOWALLDISPLAYNO = 1732;//获取设备显示输出号
    int NET_DVR_SET_VIDEOWALLDISPLAYPOSITION = 1733;//设置显示输出位置参数
    int NET_DVR_GET_VIDEOWALLDISPLAYPOSITION = 1734;//获取显示输出位置参数
    int NET_DVR_GET_VIDEOWALLWINDOWPOSITION = 1735;//获取电视墙窗口参数
    int NET_DVR_SET_VIDEOWALLWINDOWPOSITION = 1736;//设置电视墙窗口参数
    int NET_DVR_VIDEOWALLWINDOW_CLOSEALL = 1737;//电视墙关闭所有窗口
    int NET_DVR_SET_VIRTUALLED = 1738;//虚拟LED设置
    int NET_DVR_GET_VIRTUALLED = 1739;//虚拟LED获取
    int NET_DVR_GET_IMAGE_CUT_MODE = 1740;//获取图像切割模式
    int NET_DVR_SET_IMAGE_CUT_MODE = 1741;//设置图像切割模式
    int NET_DVR_GET_USING_SERIALPORT = 1742;//获取当前使用串口
    int NET_DVR_SET_USING_SERIALPORT = 1743;//设置当前使用串口
    int NET_DVR_SCENE_CONTROL = 1744;//场景控制
    int NET_DVR_GET_CURRENT_SCENE = 1745;//获取当前场景号
    int NET_DVR_GET_VW_SCENE_PARAM = 1746;//获取电视墙场景模式参数
    int NET_DVR_SET_VW_SCENE_PARAM = 1747;//设置电视墙场景模式参数
    int NET_DVR_DISPLAY_CHANNO_CONTROL = 1748;//电视墙显示编号控制
    int NET_DVR_GET_WIN_DEC_INFO = 1749;//获取窗口解码信息（批量）
    int NET_DVR_RESET_VIDEOWALLDISPLAYPOSITION = 1750;  //解除电视墙输出接口绑定
    int NET_DVR_SET_VW_AUDIO_CFG = 1752;  //设置音频切换参数
    int NET_DVR_GET_VW_AUDIO_CFG = 1753;   //获取音频切换参数
    int NET_DVR_GET_GBT28181_DECCHANINFO_CFG = 1754;   //获取GBT28181协议接入设备的解码通道信息
    int NET_DVR_SET_GBT28181_DECCHANINFO_CFG = 1755;  //设置GBT28181协议接入设备的解码通道信息
    int NET_DVR_SET_MAINBOARD_SERIAL = 1756; //设置主控板串口参数
    int NET_DVR_GET_MAINBOARD_SERIAL = 1757;//获取主控板串口参数
    int NET_DVR_GET_SUBBOARD_INFO = 1758; //获取子板信息
    int NET_DVR_GET_SUBBOARD_EXCEPTION = 1759; //获取异常子板异常信息
    /*****************************电视墙 start****************************/
    int NET_DVR_MATRIX_WALL_SET = 9001;  //设置电视墙中屏幕参数
    int NET_DVR_MATRIX_WALL_GET = 9002; //获取电视墙中屏幕参数
    int NET_DVR_WALLWIN_GET = 9003;  //电视墙中获取窗口参数
    int NET_DVR_WALLWIN_SET = 9004;  //电视墙中设置窗口参数
    int NET_DVR_WALLWINPARAM_SET = 9005;
    //设置电视墙窗口相关参数
    int NET_DVR_WALLWINPARAM_GET = 9006; //获取电视墙窗口相关参数
    int NET_DVR_WALLSCENEPARAM_GET = 9007;  //设置场景模式参数
    int NET_DVR_WALLSCENEPARAM_SET = 9008;  //获取场景模式参数
    int NET_DVR_MATRIX_GETWINSTATUS = 9009;  //获取窗口解码状态
    int NET_DVR_GET_WINASSOCIATEDDEVINFO = 9010;  //电视墙中获取对应资源信息
    int NET_DVR_WALLOUTPUT_GET = 9011;  //电视墙中获取显示输出参数
    int NET_DVR_WALLOUTPUT_SET = 9012;  //电视墙中设置显示输出参数
    int NET_DVR_GET_UNITEDMATRIXSYSTEM = 9013;  //电视墙中获取对应资源
    int NET_DVR_GET_WALL_CFG = 9014;  //获取电视墙全局参数
    int NET_DVR_SET_WALL_CFG = 9015;  //设置电视墙全局参数
    int NET_DVR_CLOSE_ALL_WND = 9016;  //关闭所有窗口
    int NET_DVR_SWITCH_WIN_TOP = 9017;  //窗口置顶
    int NET_DVR_SWITCH_WIN_BOTTOM = 9018;  //窗口置底

    int NET_DVR_CLOSE_ALL_WND_V41 = 9019;  //电视墙关闭所有窗口v41（有多个电视墙）
    int NET_DVR_GET_WALL_WINDOW_V41 = 9020;  //获取电视墙中的窗口v41
    int NET_DVR_SET_WALL_WINDOW_V41 = 9021;  //设置电视墙中的窗口v41
    int NET_DVR_GET_CURRENT_SCENE_V41 = 9022;  //获取当前电视墙中正在使用的场景v41
    int NET_DVR_GET_WALL_SCENE_PARAM_V41 = 9023;  //获取当前电视墙中正在使用的场景v41
    int NET_DVR_SET_WALL_SCENE_PARAM_V41 = 9024;  //设置当前电视墙中正在使用的场景v41
    int NET_DVR_GET_MATRIX_LOGO_CFG = 9025;  //获取logo参数
    int NET_DVR_SET_MATRIX_LOGO_CFG = 9026;  //设置logo参数
    int NET_DVR_GET_WIN_LOGO_CFG = 9027;  //获取窗口logo参数
    int NET_DVR_SET_WIN_LOGO_CFG = 9028;  //设置窗口logo参数
    int NET_DVR_DELETE_LOGO = 9029;  //删除logo
    int NET_DVR_SET_DISPLAY_EFFECT_CFG = 9030;  //设置显示输出效果参数v41
    int NET_DVR_GET_DISPLAY_EFFECT_CFG = 9031;  //获取显示输出效果参数v41
    int NET_DVR_DEC_PLAY_REMOTE_FILE = 9032;  //解码播放远程文件
    int NET_DVR_DEC_PLAY_REMOTE_FILE_V50 = 9314;  //解码播放远程文件V50
    int NET_DVR_GET_WIN_ZOOM_STATUS = 9033;  //获取窗口电子放大状态
    int NET_DVR_GET_ALL_MATRIX_LOGOCFG = 9034;  //获取所有logo参数
...
/*****************************电视墙 end******************************/
    ...
    int MINOR_LINK_START = 0xa6; // ivms多路解码器等连接前端设备
    int MINOR_LINK_STOP = 0xa7;// ivms多路解码器等断开前端设备　
    ...
    int DEVICE_ABILITY_INFO = 0x011; //设备通用能力类型，具体能力根据发送的能力节点来区分
    int NET_DVR_CHECK_USER_STATUS = 20005; //检测设备是否在线
    /**********************设备类型 end***********************/
    ...
    /////////////////////////////////////////////////////////////////////////
    //校时结构参数
    class NET_DVR_TIME extends HCNetSDKStructure {//校时结构参数
        public int dwYear;        //年
    ...
    class NET_DVR_SCHEDTIME extends HCNetSDKStructure {
    ...
    class NET_DVR_HANDLEEXCEPTION_V30 extends HCNetSDKStructure {
    ...
    //报警和异常处理结构(子结构)(多处使用)
    class NET_DVR_HANDLEEXCEPTION extends HCNetSDKStructure {
        public int dwHandleType;            /*处理方式,处理方式的"或"结果*//*0x00: 无响应*//*0x01: 监视器上警告*//*0x02: 声音警告*//*0x04: 上传中心*/    /*0x08: 触发报警输出*//*0x20: 触发抓图*/  //(JPEG定制)
    ...
    //DVR设备参数
    class NET_DVR_DEVICECFG extends HCNetSDKStructure {
        public int dwSize;
    ...
    //DVR设备参数
    class NET_DVR_DEVICECFG_V40 extends HCNetSDKStructure {
        public int dwSize;
        public byte[] sDVRName = new byte[NAME_LEN];     //DVR名称
        public int dwDVRID;                //DVR ID,用于遥控器 //V1.4(0-99), V1.5(0-255)
        public int dwRecycleRecord;        //是否循环录像,0:不是; 1:是
    ...
    class NET_DVR_IPADDR extends HCNetSDKStructure {
    ...
    //网络数据结构(子结构)(9000扩展)
    class NET_DVR_ETHERNET_V30 extends HCNetSDKStructure {
    ...
    class NET_DVR_ETHERNET extends HCNetSDKStructure {//网络数据结构(子结构)
    ...
    class NET_DVR_PPPOECFG extends HCNetSDKStructure {//PPPoe
        public int dwPPPoE;
    ...
    class NET_DVR_NETCFG_V30 extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_NETCFG extends HCNetSDKStructure {//网络配置结构
        public int dwSize;
    ...
    //通道图象结构
    class NET_DVR_SCHEDTIMEWEEK extends HCNetSDKStructure {
        public NET_DVR_SCHEDTIME[] struAlarmTime = new NET_DVR_SCHEDTIME[8];
    }

    class byte96 extends HCNetSDKStructure {
        public byte[] byMotionScope = new byte[96];
    }

    class NET_DVR_MOTION_V30 extends HCNetSDKStructure {//移动侦测(子结构)(9000扩展)
    ...
    class NET_DVR_MOTION extends HCNetSDKStructure {//移动侦测(子结构)
    ...
    class NET_DVR_HIDEALARM_V30 extends HCNetSDKStructure {//遮挡报警
    ...
    class NET_DVR_HIDEALARM extends HCNetSDKStructure {//遮挡报警(子结构)  区域大小704*576
        public int dwEnableHideAlarm;                /* 是否启动遮挡报警 ,0-否,1-低灵敏度 2-中灵敏度 3-高灵敏度*/
        public short wHideAlarmAreaTopLeftX;            /* 遮挡区域的x坐标 */
        public short wHideAlarmAreaTopLeftY;            /* 遮挡区域的y坐标 */
        public short wHideAlarmAreaWidth;                /* 遮挡区域的宽 */
    ...
    class NET_DVR_VILOST_V30 extends HCNetSDKStructure {    //信号丢失报警(子结构)(9000扩展)
        public byte byEnableHandleVILost;                         /* 是否处理信号丢失报警 */
        public NET_DVR_HANDLEEXCEPTION_V30 strVILostHandleType;         /* 处理方式 */
        public NET_DVR_SCHEDTIMEWEEK[] struAlarmTime = new NET_DVR_SCHEDTIMEWEEK[MAX_DAYS];//布防时间
    }

    class NET_DVR_VILOST extends HCNetSDKStructure {    //信号丢失报警(子结构)
        public byte byEnableHandleVILost;    /* 是否处理信号丢失报警 */
        public NET_DVR_HANDLEEXCEPTION strVILostHandleType;    /* 处理方式 */
    }

    class NET_DVR_SHELTER extends HCNetSDKStructure {  //遮挡区域(子结构)
    ...
    class NET_DVR_COLOR extends HCNetSDKStructure {
    ...
    class NET_DVR_VICOLOR extends HCNetSDKStructure {
    ...
    //信号丢失
    class NET_DVR_VILOST_V40 extends HCNetSDKStructure {
    ...
    class NET_DVR_DNMODE extends HCNetSDKStructure {
        public byte byObjectSize;//占比参数(0~100)
        public byte byMotionSensitive; /*移动侦测灵敏度, 0 - 5,越高越灵敏,0xff关闭*/
        public byte[] byRes = new byte[6];
    }

    class NET_DVR_MOTION_MULTI_AREAPARAM extends HCNetSDKStructure {
    ...
    class NET_DVR_MOTION_MULTI_AREA extends HCNetSDKStructure {
    ...
    class NET_DVR_MOTION_SINGLE_AREA extends HCNetSDKStructure {
        public byte[] byMotionScope = new byte[64 * 96];        /*侦测区域,0-96位,表示64行,共有96*64个小宏块,目前有效的是22*18,为1表示是移动侦测区域,0-表示不是*/
        public byte byMotionSensitive;            /*移动侦测灵敏度, 0 - 5,越高越灵敏,0xff关闭*/
        public byte[] byRes = new byte[3];
    }

    class NET_DVR_MOTION_MODE_PARAM extends HCNetSDKStructure {
    ...
    class NET_DVR_MOTION_V40 extends HCNetSDKStructure {
    ...
    class NET_DVR_RGB_COLOR extends HCNetSDKStructure {
    ...
    class NET_DVR_HIDEALARM_V40 extends HCNetSDKStructure {
        public int dwEnableHideAlarm;                /* 是否启动遮挡报警，0-否，1-低灵敏度，2-中灵敏度，3-高灵敏度*/
        public short wHideAlarmAreaTopLeftX;            /* 遮挡区域的x坐标 */
        public short wHideAlarmAreaTopLeftY;            /* 遮挡区域的y坐标 */
        public short wHideAlarmAreaWidth;                /* 遮挡区域的宽 */
    ...
    class NET_DVR_PICCFG_V40 extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_PICCFG_V30 extends HCNetSDKStructure {
        public int dwSize;
        public byte[] sChanName = new byte[NAME_LEN];
        public int dwVideoFormat;                /* 只读 视频制式 1-NTSC 2-PAL*/
    ...
    class NET_DVR_PICCFG_EX extends HCNetSDKStructure {//通道图象结构SDK_V14扩展
        public int dwSize;
        public byte[] sChanName = new byte[NAME_LEN];
        public int dwVideoFormat;    /* 只读 视频制式 1-NTSC 2-PAL*/
        public byte byBrightness;    /*亮度,0-255*/
    ...
    class NET_DVR_PICCFG extends HCNetSDKStructure { //通道图象结构(SDK_V13及之前版本)
        public int dwSize;
        public byte[] sChanName = new byte[NAME_LEN];
        public int dwVideoFormat;    /* 只读 视频制式 1-NTSC 2-PAL*/
        public byte byBrightness;    /*亮度,0-255*/
    ...
    class NET_DVR_MULTI_STREAM_COMPRESSIONCFG_COND extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_MULTI_STREAM_COMPRESSIONCFG extends HCNetSDKStructure {
        public int dwSize;
        public int dwStreamType;
    ...
    //码流压缩参数(子结构)(9000扩展)
    class NET_DVR_COMPRESSION_INFO_V30 extends HCNetSDKStructure {
    ...
    //通道压缩参数(9000扩展)
    class NET_DVR_COMPRESSIONCFG_V30 extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_COMPRESSION_INFO extends HCNetSDKStructure {//码流压缩参数(子结构)
    ...
    class NET_DVR_COMPRESSIONCFG extends HCNetSDKStructure {//通道压缩参数
        public int dwSize;
    ...
    class NET_DVR_COMPRESSION_INFO_EX extends HCNetSDKStructure {//码流压缩参数(子结构)(扩展) 增加I帧间隔
    ...
    class NET_DVR_RECORDSCHED extends HCNetSDKStructure //时间段录像参数配置(子结构)
    {
    ...
    class NET_DVR_RECORDDAY extends HCNetSDKStructure //全天录像参数配置(子结构)
    {
    ...
    class NET_DVR_RECORDSCHEDWEEK extends HCNetSDKStructure {
        public NET_DVR_RECORDSCHED[] struRecordSched = new NET_DVR_RECORDSCHED[MAX_TIMESEGMENT_V30];
    }

    class NET_DVR_RECORD_V30 extends HCNetSDKStructure {    //通道录像参数配置(9000扩展)
        public int dwSize;
    ...
    class NET_DVR_RECORD extends HCNetSDKStructure { //通道录像参数配置
        public int dwSize;
        public int dwRecord;  /*是否录像 0-否 1-是*/
        public NET_DVR_RECORDDAY[] struRecAllDay = new NET_DVR_RECORDDAY[MAX_DAYS];
        public NET_DVR_RECORDSCHEDWEEK[] struRecordSched = new NET_DVR_RECORDSCHEDWEEK[MAX_DAYS];
    ...
    class NET_DVR_STATFRAME extends HCNetSDKStructure { //单帧统计参数
        public int dwRelativeTime;
        public int dwAbsTime;  /*统计绝对时标*/
        public byte[] byRes = new byte[92];
    }

    class NET_DVR_STATTIME extends HCNetSDKStructure { //单帧统计参数
    ...
    class NET_DVR_PDC_ALRAM_INFO extends HCNetSDKStructure { //通道录像参数配置
        public int dwSize;
        ...
        public void read() {
            super.read();
            switch (byMode) {
                case 0 -> uStatModeParam.setType(NET_DVR_STATFRAME.class);
                case 1 -> uStatModeParam.setType(NET_DVR_STATTIME.class);
                default -> {
                }
            }
            uStatModeParam.read();
    ...
    //云台协议表结构配置
    class NET_DVR_PTZ_PROTOCOL extends HCNetSDKStructure {
        public int dwType;               /*解码器类型值，从1开始连续递增*/
        public byte[] byDescribe = new byte[DESC_LEN]; /*解码器的描述符，和8000中的一致*/
    }

    class NET_DVR_PTZCFG extends HCNetSDKStructure {
        public int dwSize;
     ...
     * 云台类型(end)
     ******************************/
    class NET_DVR_DECODERCFG_V30 extends HCNetSDKStructure {//通道解码器(云台)参数配置(9000扩展)
        public int dwSize;
    ...
    class NET_DVR_DECODERCFG extends HCNetSDKStructure {//通道解码器(云台)参数配置
        public int dwSize;
        public int dwBaudRate;       //波特率(bps)，0－50，1－75，2－110，3－150，4－300，5－600，6－1200，7－2400，8－4800，9－9600，10－19200， 11－38400，12－57600，13－76800，14－115.2k;
        public byte byDataBit;         // 数据有几位 0－5位，1－6位，2－7位，3－8位;
        public byte byStopBit;         // 停止位 0－1位，1－2位;
    ...
    class NET_DVR_PPPCFG_V30 extends HCNetSDKStructure {//ppp参数配置(子结构)
    ...
    class NET_DVR_PPPCFG extends HCNetSDKStructure {//ppp参数配置(子结构)
    ...
    class NET_DVR_SINGLE_RS232 extends HCNetSDKStructure {//RS232串口参数配置(9000扩展)
        public int dwBaudRate;   /*波特率(bps)，0－50，1－75，2－110，3－150，4－300，5－600，6－1200，7－2400，8－4800，9－9600，10－19200， 11－38400，12－57600，13－76800，14－115.2k;*/
        public byte byDataBit;     /* 数据有几位 0－5位，1－6位，2－7位，3－8位 */
        public byte byStopBit;     /* 停止位 0－1位，1－2位 */
        public byte byParity;      /* 校验 0－无校验，1－奇校验，2－偶校验 */
    ...
    class NET_DVR_RS232CFG_V30 extends HCNetSDKStructure {//RS232串口参数配置(9000扩展)
        public int dwSize;
    ...
    class NET_DVR_RS232CFG extends HCNetSDKStructure {//RS232串口参数配置
        public int dwSize;
        public int dwBaudRate;//波特率(bps)，0－50，1－75，2－110，3－150，4－300，5－600，6－1200，7－2400，8－4800，9－9600，10－19200， 11－38400，12－57600，13－76800，14－115.2k;
        public byte byDataBit;// 数据有几位 0－5位，1－6位，2－7位，3－8位;
        public byte byStopBit;// 停止位 0－1位，1－2位;
    ...
    class NET_DVR_ALARMINCFG_V30 extends HCNetSDKStructure {//报警输入参数配置(9000扩展)
        public int dwSize;
    ...
    class NET_DVR_ALARMINCFG extends HCNetSDKStructure {//报警输入参数配置
        public int dwSize;
        public byte[] sAlarmInName = new byte[NAME_LEN];    /* 名称 */
        public byte byAlarmType;    //报警器类型,0：常开,1：常闭
        public byte byAlarmInHandle;    /* 是否处理 0-不处理 1-处理*/
    ...
    class NET_DVR_ADDIT_POSITION extends HCNetSDKStructure {//车载GPS信息结构(2007-12-27)
    ...
    class struRecordingHost extends HCNetSDKStructure {
        public byte bySubAlarmType;
        public byte[] byRes1 = new byte[3];
        public NET_DVR_TIME_EX struRecordEndTime = new NET_DVR_TIME_EX();
    }

    class struAlarmHardDisk extends HCNetSDKStructure {
        public int dwAlarmHardDiskNum;
    }

    class struAlarmChannel extends HCNetSDKStructure {
    ...
    class struIOAlarm extends HCNetSDKStructure {
        public int dwAlarmInputNo;
        public int dwTrigerAlarmOutNum;
        public int dwTrigerRecordChanNum;
    }

    class NET_DVR_TIME_EX extends HCNetSDKStructure {
        public short wYear;
        public byte byMonth;
        public byte byDay;
        public byte byHour;
    ...
    class NET_DVR_ALRAM_FIXED_HEADER extends HCNetSDKStructure {
    ...
    class NET_DVR_ALARMINFO_V40 extends HCNetSDKStructure {
        public NET_DVR_ALRAM_FIXED_HEADER struAlarmFixedHeader = new NET_DVR_ALRAM_FIXED_HEADER();
        public Pointer pAlarmData;
    }

    class NET_DVR_ALARMINFO_V30 extends HCNetSDKStructure {//上传报警信息(9000扩展)
    ...
    class NET_DVR_ALARMINFO extends HCNetSDKStructure {
    ...
    class NET_DVR_ALARMINFO_EX extends HCNetSDKStructure {//上传报警信息(杭州竞天定制 2006-07-28)
    ...
    //////////////////////////////////////////////////////////////////////////////////////
    //IPC接入参数配置
    class NET_DVR_IPDEVINFO extends HCNetSDKStructure {/* IP设备结构 */
    ...
    class NET_DVR_IPCHANINFO extends HCNetSDKStructure {/* IP通道匹配参数 */
    ...
    class NET_DVR_IPPARACFG extends HCNetSDKStructure {/* IP接入配置结构 */
    ...
    class NET_DVR_IPDEVINFO_V31 extends HCNetSDKStructure {
        public byte byEnable;/* 该通道是否启用 */
    ...
    class NET_DVR_STREAM_MODE extends HCNetSDKStructure {
        ...
        public void read() {
            ...
            switch (byGetStreamType) {
                case 0 -> uGetStream.setType(NET_DVR_IPCHANINFO.class);
                case 6 -> uGetStream.setType(NET_DVR_IPCHANINFO_V40.class);
                default -> {
                }
            }
        }
    ...
    class NET_DVR_IPSERVER_STREAM extends HCNetSDKStructure {
    ...
    class NET_DVR_STREAM_MEDIA_SERVER_CFG extends HCNetSDKStructure {
    ...
    class NET_DVR_DEV_CHAN_INFO extends HCNetSDKStructure {
    ...
    class NET_DVR_PU_STREAM_CFG extends HCNetSDKStructure {
    ...
    class NET_DVR_PU_STREAM_CFG_V41 extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_DDNS_STREAM_CFG extends HCNetSDKStructure {
        public byte byEnable;
        public byte[] byRes1 = new byte[3];
    ...
    class NET_DVR_PU_STREAM_URL extends HCNetSDKStructure {
    ...
    class NET_DVR_HKDDNS_STREAM extends HCNetSDKStructure {
    ...
    class NET_DVR_IPCHANINFO_V40 extends HCNetSDKStructure {
    ...
    class NET_DVR_IPPARACFG_V40 extends HCNetSDKStructure {/* IP接入配置结构V40 */
        public int dwSize;   /* 结构大小 */
    ...
    class NET_DVR_SINGLE_HD extends HCNetSDKStructure {//本地硬盘信息配置
    ...
    class NET_DVR_SCALECFG extends HCNetSDKStructure {//配置缩放参数的结构
        public int dwSize;
    ...
    class NET_DVR_ALARMOUTCFG_V30 extends HCNetSDKStructure {//DVR报警输出(9000扩展)
        public int dwSize;
    ...
    class NET_DVR_ALARMOUTCFG extends HCNetSDKStructure {//DVR报警输出
        public int dwSize;
        public byte[] sAlarmOutName = new byte[NAME_LEN];    /* 名称 */
        public int dwAlarmOutDelay;    /* 输出保持时间(-1为无限，手动关闭) */
        //0-5秒,1-10秒,2-30秒,3-1分钟,4-2分钟,5-5分钟,6-10分钟,7-手动
    ...
    class NET_DVR_PREVIEWCFG_V30 extends HCNetSDKStructure {//DVR本地预览参数(9000扩展)
        public int dwSize;
    ...
    class NET_DVR_PREVIEWCFG extends HCNetSDKStructure {//DVR本地预览参数
        public int dwSize;
    ...
    class NET_DVR_VGAPARA extends HCNetSDKStructure {//DVR视频输出
     ...
     * MATRIX输出参数结构
     */
    class NET_DVR_MATRIXPARA_V30 extends HCNetSDKStructure {
    ...
    class NET_DVR_MATRIXPARA extends HCNetSDKStructure {
        public short wDisplayLogo;                        /* 显示视频通道号(保留) */
        public short wDisplayOsd;                        /* 显示时间(保留) */
    }

    class NET_DVR_VOOUT extends HCNetSDKStructure {
    ...
    class NET_DVR_VIDEOOUT_V30 extends HCNetSDKStructure {//DVR视频输出(9000扩展)
        public int dwSize;
    ...
    class NET_DVR_VIDEOOUT extends HCNetSDKStructure {//DVR视频输出
        public int dwSize;
    ...
    class NET_DVR_USER_INFO_V30 extends HCNetSDKStructure {//单用户参数(子结构)(9000扩展)
        public byte[] sUserName = new byte[NAME_LEN];        /* 用户名 */
        public byte[] sPassword = new byte[PASSWD_LEN];        /* 密码 */
    ...
    class NET_DVR_USER_INFO_EX extends HCNetSDKStructure {//单用户参数(SDK_V15扩展)(子结构)
        public byte[] sUserName = new byte[NAME_LEN];        /* 用户名 */
        public byte[] sPassword = new byte[PASSWD_LEN];        /* 密码 */
    ...
    class NET_DVR_USER_INFO extends HCNetSDKStructure {//单用户参数(子结构)
        public byte[] sUserName = new byte[NAME_LEN];        /* 用户名 */
        public byte[] sPassword = new byte[PASSWD_LEN];        /* 密码 */
        public int[] dwLocalRight = new int[MAX_RIGHT];    /* 权限 */
        /*数组0: 本地控制云台*/
    ...
    class NET_DVR_USER_V30 extends HCNetSDKStructure {//DVR用户参数(9000扩展)
        public int dwSize;
        public NET_DVR_USER_INFO_V30[] struUser = new NET_DVR_USER_INFO_V30[MAX_USERNUM_V30];
    }

    class NET_DVR_USER_EX extends HCNetSDKStructure {//DVR用户参数(SDK_V15扩展)
        public int dwSize;
        public NET_DVR_USER_INFO_EX[] struUser = new NET_DVR_USER_INFO_EX[MAX_USERNUM];
    }

    class NET_DVR_USER extends HCNetSDKStructure {//DVR用户参数
        public int dwSize;
        public NET_DVR_USER_INFO[] struUser = new NET_DVR_USER_INFO[MAX_USERNUM];
    }

    class NET_DVR_EXCEPTION_V30 extends HCNetSDKStructure {//DVR异常参数(9000扩展)
        public int dwSize;
    ...
    class NET_DVR_EXCEPTION extends HCNetSDKStructure {//DVR异常参数
        public int dwSize;
    ...
    class NET_DVR_CHANNELSTATE_V30 extends HCNetSDKStructure {//通道状态(9000扩展)
    ...
    class NET_DVR_CHANNELSTATE extends HCNetSDKStructure {//通道状态
        public byte byRecordStatic; //通道是否在录像,0-不录像,1-录像
        public byte bySignalStatic; //连接的信号状态,0-正常,1-信号丢失
        public byte byHardwareStatic;//通道硬件状态,0-正常,1-异常,例如DSP死掉
    ...
    class NET_DVR_DISKSTATE extends HCNetSDKStructure {//硬盘状态
        public int dwVolume;//硬盘的容量
        public int dwFreeSpace;//硬盘的剩余空间
        public int dwHardDiskStatic; //硬盘的状态,按位:1-休眠,2-不正常,3-休眠硬盘出错
    }

    class NET_DVR_WORKSTATE_V30 extends HCNetSDKStructure {//DVR工作状态(9000扩展)
    ...
    class NET_DVR_WORKSTATE extends HCNetSDKStructure {//DVR工作状态
        public int dwDeviceStatic;    //设备的状态,0-正常,1-CPU占用率太高,超过85%,2-硬件错误,例如串口死掉
    ...
    class NET_DVR_LOG_V30 extends HCNetSDKStructure {//日志信息(9000扩展)
    ...
    //日志信息
    class NET_DVR_LOG extends HCNetSDKStructure {
        public NET_DVR_TIME strLogTime;
        public int dwMajorType;    //主类型 1-报警; 2-异常; 3-操作; 0xff-全部
        public int dwMinorType;//次类型 0-全部;
        public byte[] sPanelUser = new byte[MAX_NAMELEN]; //操作面板的用户名
     ...
     * DVR日志 end
     ***************************/
    class NET_DVR_ALARMOUTSTATUS_V30 extends HCNetSDKStructure {//报警输出状态(9000扩展)
        public byte[] Output = new byte[MAX_ALARMOUT_V30];
    }

    class NET_DVR_ALARMOUTSTATUS extends HCNetSDKStructure {//报警输出状态
        public byte[] Output = new byte[MAX_ALARMOUT];
    }

    class NET_DVR_TRADEINFO extends HCNetSDKStructure {//交易信息
        public short m_Year;
    ...
    class NET_DVR_FRAMETYPECODE extends HCNetSDKStructure {/*帧格式*/
        public byte[] code = new byte[12];        /* 代码 */
    }

    class NET_DVR_FRAMEFORMAT_V30 extends HCNetSDKStructure {//ATM参数(9000扩展)
        public int dwSize;
    ...
    class NET_DVR_FRAMEFORMAT extends HCNetSDKStructure {//ATM参数
        public int dwSize;
    ...
    class NET_DVR_FTPTYPECODE extends HCNetSDKStructure {
        public byte[] sFtpType = new byte[32];     /*客户定义的操作类型*/
        public byte[] sFtpCode = new byte[8];      /*客户定义的操作类型的对应的码*/
    }

    class NET_DVR_FRAMEFORMAT_EX extends HCNetSDKStructure {//ATM参数添加FTP上传参数, 俄罗斯银行定制, 2006-11-17
        public int dwSize;
        public byte[] sATMIP = new byte[16];                        /* ATM IP地址 */
        public int dwATMType;                        /* ATM类型 */
        public int dwInputMode;                        /* 输入方式	0-网络侦听 1-网络接收 2-串口直接输入 3-串口ATM命令输入*/
...
/****************************ATM(end)***************************/
     ...
     ***************************/
//DS-6001D Decoder
    class NET_DVR_DECODERINFO extends HCNetSDKStructure {
    ...
    class NET_DVR_DECODERSTATE extends HCNetSDKStructure {
        public byte[] byEncoderIP = new byte[16];        //解码设备连接的服务器IP
        public byte[] byEncoderUser = new byte[16];        //解码设备连接的服务器的用户名
        public byte[] byEncoderPasswd = new byte[16];    //解码设备连接的服务器的密码
        public byte byEncoderChannel;        //解码设备连接的服务器的通道号
    ...
    class NET_DVR_DECCHANINFO extends HCNetSDKStructure {
        public byte[] sDVRIP = new byte[16];                /* DVR IP地址 */
        public short wDVRPort;                    /* 端口号 */
        public byte[] sUserName = new byte[NAME_LEN];        /* 用户名 */
        public byte[] sPassword = new byte[PASSWD_LEN];        /* 密码 */
    ...
    class NET_DVR_DECINFO extends HCNetSDKStructure {/*每个解码通道的配置*/
    ...
    class NET_DVR_DECCFG extends HCNetSDKStructure {/*整个设备解码配置*/
        public int dwSize;
    ...
    //2005-08-01
    class NET_DVR_PORTINFO extends HCNetSDKStructure {/* 解码设备透明通道设置 */
    ...
    class NET_DVR_PORTCFG extends HCNetSDKStructure {
        public int dwSize;
        public NET_DVR_PORTINFO[] struTransPortInfo = new NET_DVR_PORTINFO[MAX_TRANSPARENTNUM]; /* 数组0表示232 数组1表示485 */
    }

    class NET_DVR_PLAYREMOTEFILE extends HCNetSDKStructure {/* 控制网络文件回放 */
        public int dwSize;
        public byte[] sDecoderIP = new byte[16];        /* DVR IP地址 */
        public short wDecoderPort;            /* 端口号 */
        ...
        static class mode_size extends Union {
            public byte[] byFile = new byte[100];        // 回放的文件名

            static class bytime extends HCNetSDKStructure {
    ...
    class NET_DVR_DECCHANSTATUS extends HCNetSDKStructure {/*当前设备解码连接状态*/
        ...
        static class objectInfo extends Union {
            static class userInfo extends HCNetSDKStructure {
                public byte[] sUserName = new byte[NAME_LEN];    //请求视频用户名
                public byte[] sPassword = new byte[PASSWD_LEN];    // 密码
                public byte[] cReserve = new byte[52];
            }

            static class fileInfo extends HCNetSDKStructure {
                public byte[] fileName = new byte[100];
            }

            static class timeInfo extends HCNetSDKStructure {
                public int dwChannel;
                public byte[] sUserName = new byte[NAME_LEN];    //请求视频用户名
                public byte[] sPassword = new byte[PASSWD_LEN];    // 密码
                public NET_DVR_TIME struStartTime;        // 按时间回放的开始时间
    ...
    class NET_DVR_DECSTATUS extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_SHOWSTRINGINFO extends HCNetSDKStructure {//单字符参数(子结构)
...
DVR实现巡航数据结构
*/
    class NET_DVR_CRUISE_PARA extends HCNetSDKStructure {
        public int dwSize;
    ...
    //时间点
    class NET_DVR_TIMEPOINT extends HCNetSDKStructure {
    ...
    //夏令时参数
    class NET_DVR_ZONEANDDST extends HCNetSDKStructure {
        public int dwSize;
    ...
    //图片质量
    class NET_DVR_JPEGPARA extends HCNetSDKStructure {
    ...
    //辅助输出参数配置
    class NET_DVR_AUXOUTCFG extends HCNetSDKStructure {
        public int dwSize;
    ...
    //ntp
    class NET_DVR_NTPPARA extends HCNetSDKStructure {
    ...
    //ddns
    class NET_DVR_DDNSPARA extends HCNetSDKStructure {
    ...
    class NET_DVR_DDNSPARA_EX extends HCNetSDKStructure {
    ...
    class NET_DVR_DDNS extends HCNetSDKStructure {
    ...
    //9000扩展
    class NET_DVR_DDNSPARA_V30 extends HCNetSDKStructure {
    ...
    //email
    class NET_DVR_EMAILPARA extends HCNetSDKStructure {
    ...
    class NET_DVR_NETAPPCFG extends HCNetSDKStructure {//网络参数配置
        public int dwSize;
    ...
    class NET_DVR_CRUISE_RET extends HCNetSDKStructure {
    ...
    class NET_DVR_NETCFG_OTHER extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_MATRIX_DECINFO extends HCNetSDKStructure {
        public byte[] sDVRIP = new byte[16];                /* DVR IP地址 */
        public short wDVRPort;                /* 端口号 */
        public byte byChannel;                /* 通道号 */
    ...
    class NET_DVR_MATRIX_DYNAMIC_DEC extends HCNetSDKStructure {//启动/停止动态解码
        public int dwSize;
        public NET_DVR_MATRIX_DECINFO struDecChanInfo;        /* 动态解码通道信息 */
    }

    class NET_DVR_MATRIX_DEC_CHAN_STATUS extends HCNetSDKStructure {//2007-12-13 modified by zxy 修改多路解码器的NET_DVR_MATRIX_DEC_CHAN_STATUS结构
    ...
    class NET_DVR_MATRIX_DEC_CHAN_INFO extends HCNetSDKStructure {
        public int dwSize;
    ...
    //连接的通道配置 2007-11-05
    class NET_DVR_MATRIX_DECCHANINFO extends HCNetSDKStructure {
        public int dwEnable;                    /* 是否启用 0－否 1－启用*/
        public NET_DVR_MATRIX_DECINFO struDecChanInfo;        /* 轮循解码通道信息 */
    }

    //2007-11-05 新增每个解码通道的配置
    class NET_DVR_MATRIX_LOOP_DECINFO extends HCNetSDKStructure {
        public int dwSize;
    ...
    //矩阵行信息 2007-12-28
    class NET_DVR_MATRIX_ROW_ELEMENT extends HCNetSDKStructure {
    ...
    class NET_DVR_MATRIX_ROW_INDEX extends HCNetSDKStructure {
        public byte[] sSurvChanName = new byte[128];            /* 监控通道名称，支持中文 */
        public int dwRowNum;                /* 行号 */
    }

    //矩阵列信息 2007-12-28
    class NET_DVR_MATRIX_COLUMN_ELEMENT extends HCNetSDKStructure {
        public int dwLocalDispChanNum;    /* 本地显示通道号 */
        public int dwGlobalDispChanNum;    /* 全局显示通道号 */
        public int dwRes;            /* 保留 */
    }

    class NET_DVR_MATRIX_GLOBAL_COLUMN_ELEMENT extends HCNetSDKStructure {
    ...
    //手动查看 2007-12-28
    class NET_DVR_MATRIX_ROW_COLUMN_LINK extends HCNetSDKStructure {
        public int dwSize;
        /*
    ...
    class NET_DVR_MATRIX_PREVIEW_DISP_CHAN extends HCNetSDKStructure {
        public int dwSize;
        public int dwGlobalDispChanNum;        /* 电视墙上的电视机编号 */
        public int dwLocalDispChanNum;        /* 解码通道 */
    }

    class NET_DVR_MATRIX_LOOP_PLAY_SET extends HCNetSDKStructure {//轮循功能 2007-12-28
        public int dwSize;
    ...
    class NET_DVR_MATRIX_LOCAL_HOST_INFO extends HCNetSDKStructure {//矩阵中心配置 2007-12-28
        public int dwSize;
    ...
    //2007-12-22
    class TTY_CONFIG extends HCNetSDKStructure {
    ...
    class NET_DVR_MATRIX_TRAN_CHAN_INFO extends HCNetSDKStructure {
    ...
    class NET_DVR_MATRIX_TRAN_CHAN_CONFIG extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_MATRIX_DEC_REMOTE_PLAY extends HCNetSDKStructure {
        public int dwSize;
        public byte[] sDVRIP = new byte[16];        /* DVR IP地址 */
        public short wDVRPort;            /* 端口号 */
        public byte byChannel;            /* 通道号 */
    ...
    class NET_DVR_MATRIX_DEC_REMOTE_PLAY_CONTROL extends HCNetSDKStructure {
        public int dwSize;
        public int dwPlayCmd;        /* 播放命令 见文件播放命令*/
        public int dwCmdParam;        /* 播放命令参数 */
    }

    class NET_DVR_MATRIX_DEC_REMOTE_PLAY_STATUS extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_MATRIX_PASSIVEMODE extends HCNetSDKStructure {
...
/************************************多路解码器(end)***************************************/
    ...
    int NET_DVR_GET_SUBWND_DECODE_OSD = 9183;  //获取子窗口解码OSD信息
    int NET_DVR_GET_SUBWND_DECODE_OSD_ALL = 9184;  //获取所有子窗口解码OSD信息
    int NET_DVR_SET_SUBWND_DECODE_OSD = 9185;  //设置子窗口解码OSD信息
    int NET_DVR_GET_SUBWND_DECODE_OSD_CAP = 9186;  //获取子窗口解码OSD信息能力集
    int NET_DVR_GET_DECODE_CHANNEL_OSD = 9187;  //获取解码通道OSD信息
    int NET_DVR_SET_DECODE_CHANNEL_OSD = 9188;  //设置解码通道OSD信息

    int MAX_PLAN_ACTION_NUM = 32;    //预案动作个数
    int DAYS_A_WEEK = 7;    //一周7天
    int MAX_PLAN_COUNT = 16;    //预案个数
    int MAX_LEN_OSD_CONTENT = 256;  //OSD信息最大长度
    int MAX_NUM_OSD_ONE_SUBWND = 8;  //单个子窗口支持的最大OSD数量
    int MAX_NUM_SPLIT_WND = 64; //单个窗口支持的最大分屏窗口数量（即子窗口数量）
    int MAX_NUM_OSD = 8;
    int MAX_CYCLE_CHAN_V30 = 64;      //最大轮巡通道数（扩展）
    int STREAM_PASSWD_LEN = 12;      //码流加密密钥最大长度

    class NET_DVR_VIDEO_WALL_INFO extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_SCENE_CONTROL_INFO extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_BUF_INFO extends HCNetSDKStructure {
        public Pointer pBuf;
        public int nLen;
    }

    class NET_DVR_IN_PARAM extends HCNetSDKStructure {
    ...
    class NET_DVR_OUT_PARAM extends HCNetSDKStructure {
        public NET_DVR_BUF_INFO struOutBuf;
        public Pointer lpStatusList;
        public byte[] byRes = new byte[32];
    }

    class NET_DVR_RECTCFG_EX extends HCNetSDKStructure {
    ...
    class NET_DVR_VIDEOWALLWINDOWPOSITION extends HCNetSDKStructure {
        public int dwSize;
    ...
    class VIDEOWALLWINDOWPOSITION_ARRAY extends HCNetSDKStructure {
    ...
    class NET_DVR_WALLWINPARAM extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_PLAN_LIST extends HCNetSDKStructure {
        public int dwSize;
    ...
    /*预案项信息*/
    class NET_DVR_PLAN_INFO extends HCNetSDKStructure {
    ...
    class NET_DVR_CYCLE_TIME extends HCNetSDKStructure {
        public byte byValid;
        public byte[] byRes = new byte[3];
    ...
    /*预案管理*/
    class NET_DVR_PLAN_CFG extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_WALLSCENECFG extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_SUBWND_DECODE_OSD extends HCNetSDKStructure {
        public int dwSize = 0;
    ...
    class NET_DVR_OSD_INFO extends HCNetSDKStructure {
    ...
    class NET_DVR_DEV_CHAN_INFO_EX extends HCNetSDKStructure {
    ...
    class NET_DVR_STREAM_MEDIA_SERVER extends HCNetSDKStructure {
    ...
    class NET_DVR_DEV_DDNS_INFO extends HCNetSDKStructure {
    ...
    class NET_DVR_DEC_STREAM_DEV_EX extends HCNetSDKStructure {
    ...
    //DDNS方式取流
    class NET_DVR_DEC_DDNS_DEV extends HCNetSDKStructure {
        public NET_DVR_DEV_DDNS_INFO struDdnsInfo;
        public NET_DVR_STREAM_MEDIA_SERVER struMediaServer;
    }

    class NET_DVR_DEC_STREAM_MODE extends Union {
    ...
    class NET_DVR_MATRIX_CHAN_INFO_V41 extends HCNetSDKStructure {
    ...
    class NET_DVR_MATRIX_LOOP_DECINFO_V41 extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_EMAILCFG extends HCNetSDKStructure {    /* 12 bytes */
        public int dwSize;
    ...
    class NET_DVR_COMPRESSIONCFG_NEW extends HCNetSDKStructure {
        public int dwSize;
    ...
    //球机位置信息
    class NET_DVR_PTZPOS extends HCNetSDKStructure {
    ...
    //球机范围信息
    class NET_DVR_PTZSCOPE extends HCNetSDKStructure {
    ...
    class NET_DVR_PTZABSOLUTEEX_CFG extends HCNetSDKStructure {
        public int dwSize;//结构体大小
    ...
    //rtsp配置 ipcamera专用
    class NET_DVR_RTSPCFG extends HCNetSDKStructure {
    ...
    //NET_DVR_Login()参数结构
    class NET_DVR_DEVICEINFO extends HCNetSDKStructure {
        public byte[] sSerialNumber = new byte[SERIALNO_LEN];   //序列号
        public byte byAlarmInPortNum;                //DVR报警输入个数
        public byte byAlarmOutPortNum;                //DVR报警输出个数
    ...
    class NET_DVR_DEVICEINFO_V30 extends HCNetSDKStructure {
        public byte[] sSerialNumber = new byte[SERIALNO_LEN];  //序列号
    ...
    class NET_DVR_USER_LOGIN_INFO extends HCNetSDKStructure {
    ...
    //NET_DVR_Login_V40()参数
    class NET_DVR_DEVICEINFO_V40 extends HCNetSDKStructure {
    ...
    //软解码预览参数
    class NET_DVR_CLIENTINFO extends HCNetSDKStructure {
        public int lChannel;
    ...
    //预览V40接口
    class NET_DVR_PREVIEWINFO extends HCNetSDKStructure {
    ...
    class NET_DVR_STREAM_INFO extends HCNetSDKStructure {
        public int dwSize;
    ...
    //配置条件
    class NET_DVR_CLOUDSTORAGE_COND extends HCNetSDKStructure {
        public int dwSize;
        public int dwChannel;
        public byte[] byRes1 = new byte[64];
    }

    class NET_DVR_STREAM_RECORD_STATUS extends HCNetSDKStructure {
        public int dwSize;
    ...
    //SDK状态信息(9000新增)
    class NET_DVR_SDKSTATE extends HCNetSDKStructure {
    ...
    //SDK功能支持信息(9000新增)
    class NET_DVR_SDKABL extends HCNetSDKStructure {
    ...
    //报警设备信息
    class NET_DVR_ALARMER extends HCNetSDKStructure {
    ...
    //硬解码显示区域参数(子结构)
    class NET_DVR_DISPLAY_PARA extends HCNetSDKStructure {
    ...
    //硬解码预览参数
    class NET_DVR_CARDINFO extends HCNetSDKStructure {
        public int lChannel;//通道号
    ...
    //录象文件参数
    class NET_DVR_FIND_DATA extends HCNetSDKStructure {
    ...
    //录象文件参数(9000)
    class NET_DVR_FINDDATA_V30 extends HCNetSDKStructure {
        public byte[] sFileName = new byte[100];//文件名
        public NET_DVR_TIME struStartTime;//文件的开始时间
        public NET_DVR_TIME struStopTime;//文件的结束时间
        public int dwFileSize;//文件的大小
    ...
    //录象文件参数(带卡号)
    class NET_DVR_FINDDATA_CARD extends HCNetSDKStructure {
        public byte[] sFileName = new byte[100];//文件名
        public NET_DVR_TIME struStartTime;//文件的开始时间
        public NET_DVR_TIME struStopTime;//文件的结束时间
        public int dwFileSize;//文件的大小
    ...
    class NET_DVR_FILECOND_V40 extends HCNetSDKStructure {
        public int lChannel;
    ...
    class NET_DVR_ATMFINDINFO extends HCNetSDKStructure {
    ...
    //录像文件查找条件结构V50
    class NET_DVR_FILECOND_V50 extends HCNetSDKStructure {
    ...
    class NET_DVR_FINDDATA_V40 extends HCNetSDKStructure {
        public byte[] sFileName = new byte[100];//文件名
    ...
    class NET_DVR_TIME_SEARCH extends HCNetSDKStructure {
    ...
    class NET_DVR_ADDRESS extends HCNetSDKStructure {
        public NET_DVR_IPADDR struIP = new NET_DVR_IPADDR(); //IP地址
        public short wPort;    //端口号
        public byte[] byRes = new byte[2];
    }

    class NET_DVR_FINDDATA_V50 extends HCNetSDKStructure {
    ...
    class NET_DVR_FILECOND extends HCNetSDKStructure //录象文件查找条件结构
    {
        public int lChannel;//通道号
    ...
    class NET_DVR_PLAYCOND extends HCNetSDKStructure //回放或者下载信息结构体
    {
        public int dwChannel;//通道号
    ...
    class NET_DVR_VOD_PARA extends HCNetSDKStructure //回放或者下载信息结构体
    {
        public int dwSize;
    ...
    //图片查找条件
    class NET_DVR_FIND_PICTURE_PARAM extends HCNetSDKStructure {
        public int dwSize;         // 结构体大小
        public int lChannel;       // 通道号
    ...
    class NET_DVR_FIND_PICTURE extends HCNetSDKStructure {
    ...
    class NET_DVR_FIND_PICTURE_V50 extends HCNetSDKStructure {
        public byte[] sFileName = new byte[PICTURE_NAME_LEN];//图片名
    ...
    class NET_DVR_PIC_PARAM extends HCNetSDKStructure {
    ...
    //查找结果结构体
    class NET_DVR_FIND_PICTURE_V40 extends HCNetSDKStructure {
        public byte[] sFileName = new byte[PICTURE_NAME_LEN];//图片名
        public NET_DVR_TIME struTime = new NET_DVR_TIME();//图片的时间
        public int dwFileSize;//图片的大小
        public byte[] sCardNum = new byte[CARDNUM_LEN_V30];    //卡号
    ...
    //云台区域选择放大缩小(HIK 快球专用)
    class NET_DVR_POINT_FRAME extends HCNetSDKStructure {
    ...
    //语音对讲参数
    class NET_DVR_COMPRESSION_AUDIO extends HCNetSDKStructure {
        public byte byAudioEncType;   //音频编码类型 0-G722; 1-G711
        public byte[] byres = new byte[7];//这里保留音频的压缩参数
    }

    class NET_DVR_AUDIODEC_INFO extends HCNetSDKStructure {
    ...
    //音频解码
    class NET_DVR_AUDIODEC_PROCESS_PARAM extends HCNetSDKStructure {
    ...
    class NET_DVR_AUDIOENC_INFO extends HCNetSDKStructure {
        public int in_frame_size;                /* 输入一帧数据大小(BYTES)，由GetInfoParam函数返回         */
        public int[] reserved = new int[16];                 /* 保留 */
    }

    //音频编码
    class NET_DVR_AUDIOENC_PROCESS_PARAM extends HCNetSDKStructure {
    ...
    //用于接收报警信息的缓存区
    class RECV_ALARM extends HCNetSDKStructure {
        public byte[] RecvBuffer = new byte[4000];//此处的400应不小于最大报警报文长度
    }

    //布防参数
    class NET_DVR_SETUPALARM_PARAM extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_SETUPALARM_PARAM_V50 extends HCNetSDKStructure {
        public int dwSize;
        public byte byLevel; //布防优先级，0-一等级（高），1-二等级（中），2-三等级（低）
        public byte byAlarmInfoType; //上传报警信息类型（抓拍机支持），0-老报警信息（NET_DVR_PLATE_RESULT），1-新报警信息(NET_ITS_PLATE_RESULT)2012-9-28
        public byte byRetAlarmTypeV40; //0--返回NET_DVR_ALARMINFO_V30或NET_DVR_ALARMINFO, 1--设备支持NET_DVR_ALARMINFO_V40则返回NET_DVR_ALARMINFO_V40，不支持则返回NET_DVR_ALARMINFO_V30或NET_DVR_ALARMINFO
    ...
    //区域框参数
    class NET_VCA_RECT extends HCNetSDKStructure {
        public float fX;
        public float fY;
        public float fWidth;
    ...
    //报警目标信息
    class NET_VCA_TARGET_INFO extends HCNetSDKStructure {
        public int dwID;
    ...
    //前端设备信息
    class NET_VCA_DEV_INFO extends HCNetSDKStructure {
    ...
    //事件规则信息
    class NET_VCA_RULE_INFO extends HCNetSDKStructure {
        ...
        public void read() {
            super.read();
            switch (wEventTypeEx) {
                case 1 -> uEventParam.setType(NET_VCA_TRAVERSE_PLANE.class);
                case 2, 3 -> uEventParam.setType(NET_VCA_AREA.class);
                default -> {
                }
            }
            uEventParam.read();
    ...
    //穿越警戒面参数
    class NET_VCA_TRAVERSE_PLANE extends HCNetSDKStructure {
    ...
    class NET_DVR_HANDLEEXCEPTION_V40 extends HCNetSDKStructure {
    ...
    class NET_VCA_TRAVERSE_PLANE_DETECTION extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_VCA_INTRUSION extends HCNetSDKStructure {
    ...
    */
    ...
    //快速移动参数
    class NET_VCA_RUN extends HCNetSDKStructure {
        public NET_VCA_POLYGON struRegion;//区域范围
    ...
    */
    ...
    //奔跑检测
    class NET_VCA_RUNNING extends HCNetSDKStructure {
        public NET_VCA_POLYGON struRegion;  //区域范围
    ...
    //倒地参数
    class NET_VCA_FALL_DOWN extends HCNetSDKStructure {
        public NET_VCA_POLYGON struRegion;//区域范围
    ...
    int MAX_INTRUSIONREGION_NUM = 8; //最大区域数数

    class NET_VCA_FIELDDETECION extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_CHANNEL_GROUP extends HCNetSDKStructure {
        public int dwSize;
        public int dwChannel;
        public int dwGroup;
        public byte byID;
    ...
    //线结构参数
    class NET_VCA_LINE extends HCNetSDKStructure {
        public NET_VCA_POINT struStart;
        public NET_VCA_POINT struEnd;
    }

    //点坐标参数
    class NET_VCA_POINT extends HCNetSDKStructure {
        public float fX;
        public float fY;
    ...
    //进入/离开区域参数
    class NET_VCA_AREA extends HCNetSDKStructure {
        public NET_VCA_POLYGON struRegion;
        public byte[] byRes = new byte[8];
    }

    //多边形结构体
    class NET_VCA_POLYGON extends HCNetSDKStructure {
        public int dwPointNum;
        public NET_VCA_POINT[] struPos = new NET_VCA_POINT[VCA_MAX_POLYGON_POINT_NUM];
    }

    class NET_VCA_SIZE_FILTER extends HCNetSDKStructure {
    ...
    //尺寸过滤策略
    class NET_VCA_FILTER_STRATEGY extends HCNetSDKStructure {
        public byte byStrategy;      //尺寸过滤策略 0 - 不启用 1-高度和宽度过滤,2-面积过滤
        public byte[] byRes = new byte[11];       //保留
    }

    //行为分析报警
    class NET_VCA_RULE_ALARM extends HCNetSDKStructure {
        public int dwSize;
        public int dwRelativeTime;
    ...
    class NET_DVR_SYSTEM_TIME extends HCNetSDKStructure {
    ...
    class NET_AIOP_VIDEO_HEAD extends HCNetSDKStructure {
    ...
    class NET_AIOP_PICTURE_HEAD extends HCNetSDKStructure {
    ...
    class NET_DVR_AI_PICTUR_UPLOAD extends HCNetSDKStructure {
        public int dwSize;
    ...
    // AI开放平台接入轮询视频检测报警结构体。
    class NET_AIOP_POLLING_SNAP_HEAD extends HCNetSDKStructure {
    ...
    // AI开放平台接入轮询视频检测报警结构体。
    class NET_AIOP_POLLING_VIDEO_HEAD extends HCNetSDKStructure {
    ...
    //规则触发参数
    class NET_VCA_RULE_TRIGGER_PARAM extends HCNetSDKStructure {
    ...
    class NET_VCA_ONE_RULE_V42 extends HCNetSDKStructure {
    ...
    class NET_DVR_PTZ_POSITION extends HCNetSDKStructure {
    ...
    //行为分析配置结构体
    class NET_VCA_RULECFG_V42 extends HCNetSDKStructure {
    ...
    int CID_CODE_LEN = 4;
    int DEV_SERIAL_LEN = 9;
    int ACCOUNTNUM_LEN = 6;
    int ACCOUNTNUM_LEN_32 = 32;

    class NET_DVR_CID_ALARM extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_SENSOR_ALARM extends HCNetSDKStructure {
        public int dwSize;                // 结构体大小
    ...
    //开关量报警上传
    class NET_DVR_SWITCH_ALARM extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_ALARMHOST_EXCEPTION_ALARM extends HCNetSDKStructure {
        public int dwSize;             // 结构体大小
    ...
    class NET_DVR_ALARMHOST_POINT_VALUE extends HCNetSDKStructure {
    ...
    class NET_DVR_ALARMHOST_DATA_UNION extends HCNetSDKStructure {
        public byte[] byLength = new byte[40];
        public NET_DVR_ALARMHOST_POINT_VALUE struPointValue;    //监测点实时数据
    }

    class NET_DVR_ALARMHOST_DATA_UPLOAD extends HCNetSDKStructure {
        public int dwSize;
    ...
    //车牌识别结果子结构
    class NET_DVR_PLATE_INFO extends HCNetSDKStructure {
    ...
    class NET_DVR_VEHICLE_INFO extends HCNetSDKStructure {
    ...
    */
    ...
    //手动抓拍
    class NET_DVR_MANUALSNAP extends HCNetSDKStructure {
    ...
    //交通抓拍结果信息
    class NET_DVR_PLATE_RESULT extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_TIME_V30 extends HCNetSDKStructure {
        public short wYear;
        public byte byMonth;
        public byte byDay;
        public byte byHour;
    ...
    class NET_ITS_PICTURE_INFO extends HCNetSDKStructure {
    ...
    class NET_ITS_PLATE_RESULT extends HCNetSDKStructure {
        public int dwSize;
    ...
    //停车场数据上传
    class NET_ITS_PARK_VEHICLE extends HCNetSDKStructure {
    ...
    class NET_DVR_SNAPCFG extends HCNetSDKStructure {
    ...
    // 道闸控制
    class NET_DVR_BARRIERGATE_CFG extends HCNetSDKStructure {
        public int dwSize;
        public int dwChannel; //通道号
    ...
    */
    ...
    class NET_DVR_AGEGROUP_PARAM extends HCNetSDKStructure {
    ...
    class NET_DVR_SEXGROUP_PARAM extends HCNetSDKStructure {
        public int dwMale;//男（人数）
        public int dwFemale;//女（人数）
        public byte[] byRes = new byte[64];
    }

    class NET_DVR_PROGRAM_INFO extends HCNetSDKStructure {
        public int dwProgramNo; //节目编号
        public byte[] sProgramName = new byte[NAME_LEN]; //节目名称
        public byte[] byRes = new byte[16];
    }

    class NET_DVR_FACECAPTURE_STATISTICS_RESULT extends HCNetSDKStructure {
        public int dwSize;
    ...
    //获取交通数据条件结构
    class NET_DVR_TRAFFIC_DATA_QUERY_COND extends HCNetSDKStructure {
        public int dwSize;
        /*
       Bit0-通道有效
       Bit1-时间有效
    ...
    int MAX_TRAFFIC_PICTURE_NUM = 8; //交通图片数量

    //交通数据结构体
    class NET_DVR_TRAFFIC_DATA_QUERY_RESULT extends HCNetSDKStructure {
        public int dwSize;
    ...
    */
    ...
    class NET_DVR_TRAFFIC_PICTURE_PARAM extends HCNetSDKStructure {
    ...
    class NET_DVR_VEHICLE_CONTROL_COND extends HCNetSDKStructure {
        public int dwChannel;
    ...
    class NET_DVR_VEHICLE_CONTROL_LIST_INFO extends HCNetSDKStructure {
        public int dwSize;
        public int dwChannel;
        public int dwDataIndex;
    ...
    //车辆报警
    class NET_DVR_VEHICLE_CONTROL_ALARM extends HCNetSDKStructure {
        public int dwSize;
    ...
    int MAX_LED_INFO_LEN = 512;
    int MAX_VOICE_INFO_LEN = 128;

    //LED屏幕显示参数
    class NET_DVR_LEDDISPLAY_CFG extends HCNetSDKStructure {
        public int dwSize;//结构体大小
    ...
    //语音播报控制参数
    class NET_DVR_VOICEBROADCAST_CFG extends HCNetSDKStructure {
        public int dwSize;//结构体大小
    ...
    //缴费金额信息
    class NET_DVR_CHARGEACCOUNT_CFG extends HCNetSDKStructure {
        public int dwSize;//结构体大小
        public float fAccount;//实际收费金额
        public byte[] byRes = new byte[128];
    }

    int DOOR_NAME_LEN = 32;        //门名称
    int STRESS_PASSWORD_LEN = 8;   //胁迫密码长度
    int SUPER_PASSWORD_LEN = 8;    //胁迫密码长度
    int UNLOCK_PASSWORD_LEN = 8;   // 解除密码长度

    class NET_DVR_DOOR_CFG extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_DOOR_STATUS_PLAN extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_EVENT_CARD_LINKAGE_COND extends HCNetSDKStructure {
        public int dwSize;
    ...
    int MAX_ALARMHOST_ALARMIN_NUM = 512;//网络报警主机最大报警输入口数
    int MAX_ALARMHOST_ALARMOUT_NUM = 512;//网络报警主机最大报警输出口数

    class NET_DVR_EVENT_CARD_LINKAGE_CFG_V50 extends HCNetSDKStructure {
        public int dwSize;    //结构体大小
    ...
    class NET_DVR_EVENT_LINKAGE_INFO extends HCNetSDKStructure {
    ...
    class NET_DVR_EVETN_CARD_LINKAGE_UNION extends Union {
    ...
    //卡参数配置条件
    class NET_DVR_CARD_CFG_COND extends HCNetSDKStructure {
        public int dwSize;
    ...
    //获取卡参数的发送数据
    class NET_DVR_CARD_CFG_SEND_DATA extends HCNetSDKStructure {
        public int dwSize;
        public byte[] byCardNo = new byte[32];
        public byte[] byRes = new byte[16];
    }

    class CARDRIGHTPLAN extends HCNetSDKStructure {
        public byte[] byRightPlan = new byte[4];
    }

    //卡参数
    class NET_DVR_CARD_CFG extends HCNetSDKStructure {
        public int dwSize;
    ...
    int ACS_CARD_NO_LEN = 32;  //门禁卡号长度
    int MAX_GROUP_NUM_128 = 128; //最大群组数
    int MAX_DOOR_NUM_256 = 256; //最大门数
    int CARD_PASSWORD_LEN = 8;   //卡密码长度
    int MAX_CARD_READER_NUM = 64;  //最大读卡器数
    int MAX_DOOR_CODE_LEN = 8; //房间代码长度
    int MAX_LOCK_CODE_LEN = 8; //锁代码长度
    int MAX_CARD_RIGHT_PLAN_NUM = 4; //卡权限最大计划个数
    int MAX_CASE_SENSOR_NUM = 8;  //最大case sensor触发器数

    class CARDRIGHTPLAN_WORD extends HCNetSDKStructure {
        public short[] wRightPlan = new short[MAX_CARD_RIGHT_PLAN_NUM];
    }

    class NET_DVR_CARD_CFG_V50 extends HCNetSDKStructure {
        public int dwSize;
    ...
    //有效期参数结构体
    class NET_DVR_VALID_PERIOD_CFG extends HCNetSDKStructure {
        public byte byEnable;
        public byte[] byRes1 = new byte[3];
    ...
    //扩展结构体信息
    class NET_DVR_ID_CARD_INFO_EXTEND extends HCNetSDKStructure {
    ...
    //身份证信息报警
    class NET_DVR_ID_CARD_INFO_ALARM extends HCNetSDKStructure {
        public int dwSize;        //结构长度
    ...
    int CARD_READER_DESCRIPTION = 32;            //读卡器描述

    class NET_DVR_CARD_READER_CFG_V50 extends HCNetSDKStructure {
        public int dwSize;
    ...
    int NET_SDK_CONFIG_STATUS_NEED_WAIT = 1001;
    int NET_SDK_CONFIG_STATUS_FINISH = 1002;
    int NET_SDK_CONFIG_STATUS_FAILED = 1003;
    int NET_SDK_CONFIG_STATUS_EXCEPTION = 1004;

    int NET_SDK_GET_NEXT_STATUS_SUCCESS = 1000;
    int NET_SDK_GET_NEXT_STATUS_NEED_WAIT = 1001;
    int NET_SDK_NEXT_STATUS__FINISH = 1002;
    int NET_SDK_GET_NEXT_STATUS_FAILED = 1003;

    class NET_DVR_CARD_COND extends HCNetSDKStructure {
        public int dwSize;
        public int dwCardNum; //设置或获取卡数量，获取时置为0xffffffff表示获取所有卡信息
        public byte[] byRes = new byte[64];
    }

    class NET_DVR_CARD_SEND_DATA extends HCNetSDKStructure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //卡号
        public byte[] byRes = new byte[16];
    }

    class NET_DVR_CARD_RECORD extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_CARD_STATUS extends HCNetSDKStructure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN];
    ...
    class NET_DVR_FACE_COND extends HCNetSDKStructure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN];
    ...
    class NET_DVR_FACE_RECORD extends HCNetSDKStructure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN];
    ...
    class NET_DVR_FACE_STATUS extends HCNetSDKStructure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN];
    ...
    class NET_DVR_FINGERPRINT_COND extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_FINGERPRINT_RECORD extends HCNetSDKStructure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN];
    ...
    class NET_DVR_FINGERPRINT_STATUS extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_CAPTURE_FINGERPRINT_COND extends HCNetSDKStructure {
        public int dwSize;
    ...
    //
    class NET_DVR_CAPTURE_FINGERPRINT_CFG extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_FINGER_PRINT_INFO_CTRL_V50 extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_DEL_FINGER_PRINT_MODE_V50 extends Union {
    ...
    class NET_DVR_FINGER_PRINT_BYREADER_V50 extends HCNetSDKStructure {
    ...
    class NET_DVR_FINGER_PRINT_BYCARD_V50 extends HCNetSDKStructure {
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //指纹关联的卡号
    ...
    //人脸删除控制参数结构体
    class NET_DVR_FACE_PARAM_CTRL extends HCNetSDKStructure {
        public int dwSize;
        ...
        public void read() {
            super.read();
            switch (byMode) {
                case 0 -> struProcessMode.setType(NET_DVR_FACE_PARAM_BYCARD.class);
                case 1 -> struProcessMode.setType(NET_DVR_FACE_PARAM_BYREADER.class);
                default -> {
                }
            }
    ...
    //指纹删除控制参数结构体
    class NET_DVR_FINGER_PRINT_INFO_CTRL extends HCNetSDKStructure {
        public int dwSize;
        public byte byMode;          //删除方式，0-按卡号方式删除，1-按读卡器删除
        public byte[] byRes1 = new byte[3];        //保留
        ...
        public void read() {
            super.read();
            switch (byMode) {
                case 0 -> struProcessMode.setType(NET_DVR_FINGER_PRINT_BYCARD.class);
                case 1 -> struProcessMode.setType(NET_DVR_FINGER_PRINT_BYREADER.class);
                default -> {
                }
            }
            struProcessMode.read();
    ...
    class NET_DVR_FINGER_PRINT_BYREADER extends HCNetSDKStructure {
    ...
    //门禁主机参数
    class NET_DVR_ACS_CFG extends HCNetSDKStructure {
        public int dwSize;            //结构体大小
     ...
     * 优化接口结构体定义结束
     ***************************/
    class NET_DVR_UPLOAD_ID_BLOCKLIST_COND extends HCNetSDKStructure {
        public int dwSize;
        public int dwBlockListNum; //禁止名单数量
        public byte[] byRes = new byte[128];
    }

    class NET_DVR_UPLOAD_ID_BLOCKLIST_CFG extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_UPLOAD_ID_BLOCKLIST_STATUS extends HCNetSDKStructure {
        public int dwSize;
    ...
    class REMOTECONFIGSTATUS extends HCNetSDKStructure {
    ...
    //开锁记录
    class NET_DVR_UNLOCK_RECORD_INFO extends HCNetSDKStructure {
    ...
    //公告信息阅读回执
    class NET_DVR_NOTICEDATA_RECEIPT_INFO extends HCNetSDKStructure {
        public byte[] byNoticeNumber = new byte[MAX_NOTICE_NUMBER_LEN]; //公告编号
        public byte[] byRes = new byte[224];  //保留
    }

    //认证记录（设备未实现）
    class NET_DVR_AUTH_INFO extends HCNetSDKStructure {
    ...
    //车牌信息上传
    class NET_DVR_UPLOAD_PLATE_INFO extends HCNetSDKStructure {
        public byte[] sLicense = new byte[MAX_LICENSE_LEN];        //车牌号码
        public byte byColor;                      //车牌颜色，参考结构VCA_PLATE_COLOR
        public byte[] byRes = new byte[239];  //保留
    }

    class NET_DVR_SEND_CARD_INFO extends HCNetSDKStructure {
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN/*32*/]; //卡号
        public byte[] byRes = new byte[224];  //保留
    }

    //可视对讲事件记录信息联合体
    class NET_DVR_VIDEO_INTERCOM_EVENT_INFO_UINON extends Union {
    ...
    //可视对讲事件记录
    class NET_DVR_VIDEO_INTERCOM_EVENT extends HCNetSDKStructure {
        public int dwSize; //结构体大小
    ...
    class NET_DVR_CONTROL_GATEWAY extends HCNetSDKStructure {
        public int dwSize; //结构体大小
    ...
    //公告图片信息结构体
    class NET_DVR_NOTICE_PIC extends HCNetSDKStructure {
    ...
    //公告数据
    class NET_DVR_NOTICE_DATA extends HCNetSDKStructure {
        public int dwSize; //结构体大小
    ...
    class NET_DVR_DATE extends HCNetSDKStructure {
        public short wYear;        //年
    ...
    //身份证信息
    class NET_DVR_ID_CARD_INFO extends HCNetSDKStructure {
        public int dwSize;        //结构长度
    ...
    class NET_DVR_ACS_EVENT_INFO_EXTEND_V20 extends HCNetSDKStructure {
        public byte byRemoteCheck; //是否需要远程核验（0-无效，1-不需要（默认），2-需要）
        public byte byThermometryUnit; //测温单位（0-摄氏度（默认），1-华氏度，2-开尔文）
        public byte byIsAbnomalTemperature; //人脸抓拍测温是否温度异常：1-是，0-否
        public byte byRes2;
    ...
    //门禁主机报警信息结构体
    class NET_DVR_ACS_ALARM_INFO extends HCNetSDKStructure {
        public int dwSize;
        public int dwMajor; //报警主类型，参考宏定义
        public int dwMinor; //报警次类型，参考宏定义
    ...
    //门禁主机事件信息
    class NET_DVR_ACS_EVENT_INFO extends HCNetSDKStructure {
        public int dwSize;
        public byte[] byCardNo = new byte[32];
        public byte byCardType;
    ...
    int NET_DEV_NAME_LEN = 64;

    class NET_DVR_ACS_EVENT_INFO_EXTEND extends HCNetSDKStructure {
    ...
    门禁主机报警事件细节结构体
     */
    class NET_DVR_ACS_EVENT_DETAIL extends HCNetSDKStructure {
        public int dwSize;
    ...
    门禁主机报警事件配置结构体
     */
    class NET_DVR_ACS_EVENT_CFG extends HCNetSDKStructure {
        public int dwSize;
        public int dwMajor; //报警主类型，参考宏定义
        public int dwMinor; //报警次类型，参考宏定义
        public NET_DVR_TIME struTime = new NET_DVR_TIME(); //时间
    ...
    int NET_SDK_MONITOR_ID_LEN = 64;

    class NET_DVR_ACS_EVENT_COND extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_ACS_WORK_STATUS_V50 extends HCNetSDKStructure {
        public int dwSize;
    ...
    int ACS_PARAM_DOOR_STATUS_WEEK_PLAN = 0x00000001;//门状态周计划参数
    int ACS_PARAM_VERIFY_WEEK_PALN = 0x00000002; //读卡器周计划参数
    int ACS_PARAM_CARD_RIGHT_WEEK_PLAN = 0x00000004; //卡权限周计划参数
    int ACS_PARAM_DOOR_STATUS_HOLIDAY_PLAN = 0x00000008; //门状态假日计划参数
    int ACS_PARAM_VERIFY_HOLIDAY_PALN = 0x00000010; //读卡器假日计划参数
    int ACS_PARAM_CARD_RIGHT_HOLIDAY_PLAN = 0x00000020; //卡权限假日计划参数
    int ACS_PARAM_DOOR_STATUS_HOLIDAY_GROUP = 0x00000040; //门状态假日组参数
    int ACS_PARAM_VERIFY_HOLIDAY_GROUP = 0x00000080;//读卡器验证方式假日组参数
    int ACS_PARAM_CARD_RIGHT_HOLIDAY_GROUP = 0x00000100; //卡权限假日组参数
    int ACS_PARAM_DOOR_STATUS_PLAN_TEMPLATE = 0x00000200;//门状态计划模板参数
    int ACS_PARAM_VERIFY_PALN_TEMPLATE = 0x00000400; //读卡器验证方式计划模板参数
    int ACS_PARAM_CARD_RIGHT_PALN_TEMPLATE = 0x00000800; //卡权限计划模板参数
    int ACS_PARAM_CARD = 0x00001000; //卡参数
    int ACS_PARAM_GROUP = 0x00002000; //群组参数
    int ACS_PARAM_ANTI_SNEAK_CFG = 0x00004000; //反潜回参数
    int ACS_PAPAM_EVENT_CARD_LINKAGE = 0x00008000; //事件及卡号联动参数
    int ACS_PAPAM_CARD_PASSWD_CFG = 0x00010000; //密码开门使能参数
    int ACS_PARAM_PERSON_STATISTICS_CFG = 0x00020000; //人数统计参数
    int ACS_PARAM_BLOCKLIST_PICTURE = 0x00040000; //禁止名单图片参数
    int ACS_PARAM_ID_BLOCKLIST = 0x00080000; //身份证禁止名单参数
    int ACS_PARAM_EXAM_INFO = 0x00100000; //考试信息参数
    int ACS_PARAM_EXAMINEE_INFO = 0x00200000; //考生信息参数
    int ACS_PARAM_FAILED_FACE_INFO = 0x00400000; //升级设备人脸建模失败记录

    class NET_DVR_ACS_PARAM_TYPE extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_FACE_PARAM_COND extends HCNetSDKStructure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN];    //人脸关联的卡号
    ...
    class NET_DVR_FACE_PARAM_CFG extends HCNetSDKStructure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN];    //人脸关联的卡号
    ...
    class NET_DVR_FACE_PARAM_STATUS extends HCNetSDKStructure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //人脸关联的卡号
    ...
    class NET_DVR_FACE_PARAM_BYCARD extends HCNetSDKStructure {
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //人脸关联的卡号
        public byte[] byEnableCardReader = new byte[MAX_CARD_READER_NUM_512];  //人脸的读卡器信息，按数组表示
        public byte[] byFaceID = new byte[MAX_FACE_NUM];        //需要删除的人脸编号，按数组下标，值表示0-不删除，1-删除该人脸
    ...
    class NET_DVR_FACE_PARAM_BYREADER extends HCNetSDKStructure {
        public int dwCardReaderNo;  //按值表示，人脸读卡器编号
        public byte byClearAllCard;  //是否删除所有卡的人脸信息，0-按卡号删除人脸信息，1-删除所有卡的人脸信息
        public byte[] byRes1 = new byte[3];       //保留
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //人脸关联的卡号
    ...
    class NET_DVR_CHECK_FACE_PICTURE_COND extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_CHECK_FACE_PICTURE_CFG extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_CHECK_FACE_PICTURE_STATUS extends HCNetSDKStructure {
        public int dwSize;
        public int dwPictureNo; //图片编号
    ...
    class NET_DVR_FINGER_PRINT_CFG_V50 extends HCNetSDKStructure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //指纹关联的卡号
        public int dwFingerPrintLen;    //指纹数据长度
    ...
    class NET_DVR_FINGER_PRINT_STATUS_V50 extends HCNetSDKStructure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //指纹关联的卡号
    ...
    class NET_DVR_FINGER_PRINT_INFO_COND_V50 extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_GROUP_CFG extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_MULTI_CARD_CFG_V50 extends HCNetSDKStructure {
        public int dwSize;
        public byte byEnable;
    ...
    class NET_DVR_MULTI_CARD_GROUP_CFG_V50 extends HCNetSDKStructure {
        public byte byEnable;
    ...
    class NET_DVR_GROUP_COMBINATION_INFO_V50 extends HCNetSDKStructure {
        public byte byEnable;
    ...
    //自定义结构体，用于二维数组转换
    class NET_DVR_SINGLE_PLAN_SEGMENT_WEEK extends HCNetSDKStructure {
        public NET_DVR_SINGLE_PLAN_SEGMENT[] struPlanCfgDay = new NET_DVR_SINGLE_PLAN_SEGMENT[MAX_TIMESEGMENT_V30]; //一天的计划参数
    }

    class NET_DVR_WEEK_PLAN_CFG extends HCNetSDKStructure {
        public int dwSize;
        public byte byEnable;  //是否使能，1-使能，0-不使能
        public byte[] byRes1 = new byte[3];
    ...
    class NET_DVR_SINGLE_PLAN_SEGMENT extends HCNetSDKStructure {
        public byte byEnable; //是否使能，1-使能，0-不使能
    ...
    class NET_DVR_TIME_SEGMENT extends HCNetSDKStructure {
        public NET_DVR_SIMPLE_DAYTIME struBeginTime; //开始时间点
        public NET_DVR_SIMPLE_DAYTIME struEndTime;   //结束时间点
    }

    class NET_DVR_SIMPLE_DAYTIME extends HCNetSDKStructure {
    ...
    class NET_DVR_WEEK_PLAN_COND extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_PLAN_TEMPLATE extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_PLAN_TEMPLATE_COND extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_CAPTURE_FACE_COND extends HCNetSDKStructure {
        public int dwSize;
        public byte[] byRes = new byte[128];
    }

    class NET_DVR_FACE_FEATURE extends HCNetSDKStructure {
    ...
    class NET_DVR_CAPTURE_FACE_CFG extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_XML_CONFIG_INPUT extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_STRING_POINTER extends HCNetSDKStructure {
    ...
    class NET_DVR_XML_CONFIG_OUTPUT extends HCNetSDKStructure {
        public int dwSize;
    ...
    //报警场景信息
    class NET_DVR_SCENE_INFO extends HCNetSDKStructure {
    ...
    //交通取证报警
    class NET_DVR_TFS_ALARM extends HCNetSDKStructure {
        public int dwSize;                //结构体大小
    ...
    class NET_ITS_OVERLAPCFG_COND extends HCNetSDKStructure {
        public int dwSize;
        public int dwChannel;
    ...
    //字符叠加每一条信息结构体
    class NET_ITS_OVERLAP_SINGLE_ITEM_PARAM_V50 extends HCNetSDKStructure {
        public byte[] byRes1 = new byte[2];                   // 保留
    ...
    int MAX_OVERLAP_ITEM_NUM = 50; //最大字符叠加种数

    class NET_ITS_OVERLAP_ITEM_PARAM_V50 extends HCNetSDKStructure {
    ...
    //叠加项具体信息
    class NET_ITS_OVERLAP_INFO_PARAM extends HCNetSDKStructure {
    ...
    class NET_ITS_OVERLAP_CFG_V50 extends HCNetSDKStructure {
        public int dwSize;
        public byte byEnable; //是否启用，0-不启用，1-启用
        public byte[] byRes1 = new byte[3];
    ...
    //人体特征识别结果结构体
    class NET_VCA_HUMAN_FEATURE extends HCNetSDKStructure {
    ...
    //人脸抓拍附加信息结构体
    class NET_VCA_FACESNAP_ADDINFO extends HCNetSDKStructure {
    ...
    //人脸抓拍结果
    class NET_VCA_FACESNAP_RESULT extends HCNetSDKStructure {
        public int dwSize;             // 结构大小
        public int dwRelativeTime;     // 相对时标
        public int dwAbsTime;            // 绝对时标
    ...
    //人脸抓拍信息
    class NET_VCA_FACESNAP_INFO_ALARM extends HCNetSDKStructure {
        public int dwRelativeTime;     // 相对时标
        public int dwAbsTime;            // 绝对时标
    ...
    //籍贯参数
    class NET_DVR_AREAINFOCFG extends HCNetSDKStructure {
    ...
    class NET_VCA_HUMAN_ATTRIBUTE extends HCNetSDKStructure {
    ...
    //禁止名单报警信息
    class NET_VCA_BLOCKLIST_INFO_ALARM extends HCNetSDKStructure {
    ...
    //禁止名单信息
    class NET_VCA_BLOCKLIST_INFO extends HCNetSDKStructure {
        public int dwSize;   //结构大小
    ...
    //禁止名单比对结果报警上传
    class NET_VCA_FACESNAP_MATCH_ALARM extends HCNetSDKStructure {
        public int dwSize;             // 结构大小
    ...
    //交通事件报警(扩展)
    class NET_DVR_AID_ALARM_V41 extends HCNetSDKStructure {
        public int dwSize;              //结构长度
        public int dwRelativeTime;        //相对时标
        public int dwAbsTime;            //绝对时标
        public NET_VCA_DEV_INFO struDevInfo = new NET_VCA_DEV_INFO();            //前端设备信息
    ...
    //交通统计信息报警(扩展)
    class NET_DVR_TPS_ALARM_V41 extends HCNetSDKStructure {
        public int dwSize;          // 结构体大小
        public int dwRelativeTime;  // 相对时标
        public int dwAbsTime;       // 绝对时标
    ...
    class NET_DVR_LANE_PARAM_V41 extends HCNetSDKStructure {
    ...
    class NET_DVR_TPS_INFO_V41 extends HCNetSDKStructure {
    ...
    // 车道队列结构体
    class NET_DVR_LANE_QUEUE extends HCNetSDKStructure {
    ...
    //TPS统计过车数据上传
    class NET_DVR_TPS_STATISTICS_INFO extends HCNetSDKStructure {
        public int dwSize;          // 结构体大小
    ...
    // 交通参数统计信息
    class NET_DVR_TPS_STATISTICS_PARAM extends HCNetSDKStructure {
    ...
    //统计信息
    class NET_DVR_TPS_LANE_PARAM extends HCNetSDKStructure {
    ...
    //TPS实时过车数据上传
    class NET_DVR_TPS_REAL_TIME_INFO extends HCNetSDKStructure {
        public int dwSize;          // 结构体大小
        public int dwChan;//通道号
    ...
    //实时信息
    class NET_DVR_TPS_PARAM extends HCNetSDKStructure {
        public byte byStart;          // 开始码
    ...
    class NET_DVR_TIME_SEARCH_COND extends HCNetSDKStructure {
        public short wYear; //年
        public byte byMonth; //月
        public byte byDay; //日
        public byte byHour; //时
    ...
    //事件搜索条件
    class NET_DVR_SEARCH_EVENT_PARAM extends HCNetSDKStructure {
    ...
    //审讯事件搜索条件
    class EVENT_INQUESTPARAM extends HCNetSDKStructure {
    ...
    //事件搜索条件
    class NET_DVR_SEARCH_EVENT_PARAM_V50 extends HCNetSDKStructure {
        public short wMajorType;            //0-移动侦测，1-报警输入, 2-智能事件 5-pos录像 7-门禁事件, 8-非视频联动事件
        public short wMinorType;            //搜索次类型- 根据主类型变化，0xffff表示全部
    ...
    class EVENT_ALARMPARAM_V50 extends HCNetSDKStructure {
    ...
    //移动侦测
    class EVENT_MOTIONPARAM_V50 extends HCNetSDKStructure {
    ...
    //行为分析
    class EVENT_VCAPARAM_V50 extends HCNetSDKStructure {
    ...
    //审讯事件搜索条件
    class EVENT_INQUESTPARAM_V50 extends HCNetSDKStructure {
        public byte byRoomIndex;    //审讯室编号,从1开始
        public byte[] byRes = new byte[799];     //保留
    }

    //智能侦测查找条件 ，通道号按值表示
    class EVENT_VCADETECTPARAM_V50 extends HCNetSDKStructure {
    ...
    class EVENT_STREAMIDPARAM_V50 extends HCNetSDKStructure {
    ...
    //pos录像
    class EVENT_POSPARAM_V50 extends HCNetSDKStructure {
    ...
    class EVENT_TRIALPARAM_V50 extends HCNetSDKStructure {
    ...
    //门禁事件搜索条件
    class EVENT_ACSPARAM_V50 extends HCNetSDKStructure {
    ...
    //非视频联动事件搜索条件
    class EVENT_IOTPARAM_V50 extends HCNetSDKStructure {
    ...
    //查找返回结果
    class NET_DVR_SEARCH_EVENT_RET extends HCNetSDKStructure {
    ...
    //报警输入结果
    class EVENT_ALARMSTRET extends HCNetSDKStructure {
    ...
    //审讯事件
    class EVENT_INQUESTRET extends HCNetSDKStructure {
        public byte byRoomIndex;     //审讯室编号,从1开始
    ...
    //查找返回结果
    class NET_DVR_SEARCH_EVENT_RET_V50 extends HCNetSDKStructure {
        public short wMajorType;            //主类型
        public short wMinorType;            //次类型
    ...
    //报警输入结果
    class EVENT_ALARMRET_V50 extends HCNetSDKStructure {
        public int dwAlarmInNo;     //报警输入号
        public byte[] byRes = new byte[796];
    }

    //移动侦测结果
    class EVENT_MOTIONRET_V50 extends HCNetSDKStructure {
        public int dwMotDetNo;    //移动侦测通道
        public byte[] byRes = new byte[796];
    }

    //行为分析结果
    class EVENT_VCARET_V50 extends HCNetSDKStructure {
    ...
    //审讯事件
    class EVENT_INQUESTRET_V50 extends HCNetSDKStructure {
        public byte byRoomIndex;     //审讯室编号,从1开始
        public byte byDriveIndex;    //刻录机编号,从1开始
        public byte[] byRes1 = new byte[6];       //保留
        public int dwSegmentNo;     //本片断在本次审讯中的序号,从1开始
    ...
    //流id录像查询结果
    class EVENT_STREAMIDRET_V50 extends HCNetSDKStructure {
    ...
    //POS录像查询结果
    class EVENT_POSRET_V50 extends HCNetSDKStructure {
        public int dwChanNo;        //触发产生pos事件的通道
        public byte[] byRes = new byte[796];
    }

    class EVENT_TRIALRET_V50 extends HCNetSDKStructure {
        public byte byRoomIndex;     //审讯室编号,从1开始
        public byte byDriveIndex;    //刻录机编号,从1开始
        public short wSegmetSize;     //本片断的大小, 单位M
        public int dwSegmentNo;    //本片断在本次审讯中的序号,从1开始
    ...
    //非视频通道查询结果
    class EVENT_IOTRET_V50 extends HCNetSDKStructure {
        public int dwChanNo;        //触发产生事件的通道号（事件源通道）
        public byte[] byRes = new byte[796];
    }

    class NET_DVR_INQUEST_RESUME_SEGMENT extends HCNetSDKStructure {
    ...
    class NET_DVR_INQUEST_RESUME_EVENT extends HCNetSDKStructure {
    ...
    //报警信息查询条件结构体
    class NET_DVR_ALARM_SEARCH_COND extends HCNetSDKStructure {
        public int dwSize;
    ...
    //报警信息查询结果结构体
    class NET_DVR_ALARM_SEARCH_RESULT extends HCNetSDKStructure {
        public int dwSize;
        /*
  	报警命令，该字段值与报警布防类型相同，目前支持：
  	COMM_VCA_ALARM 	0x4993    智能检测报警
    ...
    class NET_DVR_ALARM_ISAPI_INFO extends HCNetSDKStructure {
    ...
    class NET_DVR_LOCAL_GENERAL_CFG extends HCNetSDKStructure {
    ...
    class NET_DVR_LOCAL_TCP_PORT_BIND_CFG extends HCNetSDKStructure {
    ...
    class NET_DVR_LOCAL_CHECK_DEV extends HCNetSDKStructure {
    ...
    class NET_DVR_ALARM_ISAPI_PICDATA extends HCNetSDKStructure {
        public int dwPicLen;
    ...
    class NET_DVR_FOCUSMODE_CFG extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_SERIALSTART_V40 extends HCNetSDKStructure {
        public int dwSize;         //结构体大小
    ...
    class NET_DVR_PRESET_NAME extends HCNetSDKStructure {
        public int dwSize;
    ...
    //Sensor信息
    class NET_DVR_SENSOR_PARAM extends HCNetSDKStructure {
    ...
    //球机位置信息
    class NET_DVR_PTZPOS_PARAM extends HCNetSDKStructure {
    ...
    class NET_DVR_LLI_PARAM extends HCNetSDKStructure {
    ...
    //GIS信息上传
    class NET_DVR_GIS_UPLOADINFO extends HCNetSDKStructure {
        public int dwSize;//结构体大小
        public int dwRelativeTime; //相对时标
        public int dwAbsTime; //绝对时标
    ...
    class NET_DVR_DAYTIME extends HCNetSDKStructure {
    ...
    class NET_DVR_SCHEDULE_DAYTIME extends HCNetSDKStructure {
        public NET_DVR_DAYTIME struStartTime; //开始时间
        public NET_DVR_DAYTIME struStopTime; //结束时间
    }

    class NET_DVR_BUILTIN_SUPPLEMENTLIGHT extends HCNetSDKStructure {
        public int dwSize;//结构体大小
    ...
    class NET_DVR_HANDLEEXCEPTION_V41 extends HCNetSDKStructure {
        public int dwHandleType;        //异常处理,异常处理方式的"或"结果
        /*0x00: 无响应*/
        /*0x01: 监视器上警告*/
        /*0x02: 声音警告*/
    ...
    class NET_DVR_FACELIB_GUARD_COND extends HCNetSDKStructure {
        public int dwSize;
        public int dwChannel;  //通道号
    ...
    //导入人脸数据条件
    class NET_DVR_FACELIB_COND extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_SEND_PARAM_IN extends HCNetSDKStructure {
    ...
    class NET_DVR_INQUEST_ROOM extends HCNetSDKStructure {
        public byte byRoomIndex;     //审讯室编号
        public byte byFileType;        //0-审讯文件，1-开庭上传文件
        public byte[] byRes = new byte[22];       //保留
    }

    class NET_DVR_INQUEST_CDRW_CFG extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_INQUEST_CDRW_STATUS extends HCNetSDKStructure {
        /*运行状态：0-审讯开始，
     1-审讯过程中刻录，2-审讯停止，
     3-刻录审讯文件,
     4-备份(事后备份和本地备份)
    ...
    class NET_DVR_INQUEST_CDRW extends HCNetSDKStructure {
    ...
    //实时温度检测条件参数
    class NET_DVR_REALTIME_THERMOMETRY_COND extends HCNetSDKStructure {
        public int dwSize; /*结构体大小*/
    ...
    //点测温实时信息
    class NET_DVR_POINT_THERM_CFG extends HCNetSDKStructure {
    ...
    //框/线测温实时信息
    class NET_DVR_LINEPOLYGON_THERM_CFG extends HCNetSDKStructure {
    ...
    //实时温度信息
    class NET_DVR_THERMOMETRY_UPLOAD extends HCNetSDKStructure {
        public int dwSize; /* 结构体大小 */
        public int dwRelativeTime;
        public int dwAbsTime;
    ...
    class NET_PTZ_INFO extends HCNetSDKStructure {
        public float fPan;
        public float fTilt;
        public float fZoom;
    ...
    //测温模式配置
    class NET_DVR_THERMOMETRY_MODE extends HCNetSDKStructure {
        public int dwSize;//结构体大小
    ...
    class NET_DVR_THERMOMETRY_COND extends HCNetSDKStructure {
        public int dwSize;//结构体大小
        public int dwChannel;
    ...
    class NET_DVR_THERMOMETRY_PRESETINFO_PARAM extends HCNetSDKStructure {
    ...
    class NET_DVR_THERMOMETRY_PRESETINFO extends HCNetSDKStructure {
        public int dwSize;//结构体大小
        public short wPresetNo;//0-保留
        public byte[] byRes = new byte[2];
    ...
    //温度报警（检测温度和配置温度比较报警）
    class NET_DVR_THERMOMETRY_ALARM extends HCNetSDKStructure {
        public int dwSize;
        public int dwChannel;//通道号
        public byte byRuleID;//规则ID
        public byte byThermometryUnit;//测温单位: 0-摄氏度（℃），1-华氏度（℉），2-开尔文(K)
    ...
    //温差报警
    class NET_DVR_THERMOMETRY_DIFF_ALARM extends HCNetSDKStructure {
        public int dwSize;
        public int dwChannel;//通道号
    ...
    //船只检测报警上传
    class NET_DVR_SHIPSDETECTION_ALARM extends HCNetSDKStructure {
        public int dwSize;
    ...
    int MAX_SHIPS_NUM = 20;   //船只检测最大船只数

    //船只信息
    class NET_DVR_SHIPSINFO extends HCNetSDKStructure {
    ...
    // 阵列信息
    class NET_DVR_ARRAY_INFO extends HCNetSDKStructure {
    ...
    //物理磁盘
    class NET_DVR_PHY_DISK_INFO extends HCNetSDKStructure {
    ...
    class NET_DVR_WORKSTATE_V40 extends HCNetSDKStructure {
        public int dwSize;            //结构体大小
        public int dwDeviceStatic;      //设备的状态,0-正常,1-CPU占用率太高,超过85%,2-硬件错误,例如串口死掉
    ...
    class NET_DVR_GETWORKSTATE_COND extends HCNetSDKStructure {
    ...
    //多边型结构体
    class NET_ITC_POLYGON extends HCNetSDKStructure {
    ...
    class NET_ITC_PLATE_RECOG_REGION_PARAM extends HCNetSDKStructure {
    ...
    //单组IO测速参数
    class NET_ITC_SINGLE_IOSPEED_PARAM extends HCNetSDKStructure {
        public byte byEnable; //是否启用，0-不启用，1-启用
    ...
    //牌识参数
    class NET_ITC_PLATE_RECOG_PARAM extends HCNetSDKStructure {
    ...
    //卡口IO测速参数
    class NET_ITC_POST_IOSPEED_PARAM extends HCNetSDKStructure {
    ...
    class NET_DVR_GEOGLOCATION extends HCNetSDKStructure {
        public int[] iRes = new int[2]; /*保留*/
        public int dwCity; /*城市，详见PROVINCE_CITY_IDX */
    }

    class NET_ITC_INTERVAL_PARAM extends HCNetSDKStructure {
        public byte byIntervalType;    //间隔类型（默认按时间），0-时间起效,1-距离起效
        public byte[] byRes1 = new byte[3];
    ...
    class NET_ITC_VTLANE_PARAM extends HCNetSDKStructure {
        public byte byRelatedDriveWay;//关联的车道号
        public byte bySpeedCapEn; //是否启用超速抓拍，0-否，1-是
    ...
    class NET_ITC_VTCOIL_INFO extends HCNetSDKStructure {
    ...
    class NET_ITC_RADAR_PARAM extends HCNetSDKStructure {
    ...
    //卡口虚拟线圈触发参数
    class NET_ITC_POST_VTCOIL_PARAM extends HCNetSDKStructure {
    ...
    //车道属性参数结构
    class NET_ITC_LANE_LOGIC_PARAM extends HCNetSDKStructure {
        public byte byUseageType;     //车道用途类型，详见ITC_LANE_USEAGE_TYPE
    ...
    //视频电警线结构
    class NET_ITC_LINE extends HCNetSDKStructure {
        public NET_VCA_LINE struLine = new NET_VCA_LINE(); //线参数
        public byte byLineType; //线类型，详见ITC_LINE_TYPE
        public byte[] byRes = new byte[7];
    }

    class NET_ITC_SNAPMODE_PARAM extends HCNetSDKStructure {
    ...
    //size = 128
    class NET_ITC_HVT_EC_PARAM extends HCNetSDKStructure {
    ...
    class NET_ITC_LANE_HVT_PARAM extends HCNetSDKStructure {
    ...
    class NET_ITC_POST_HVT_PARAM extends HCNetSDKStructure {
    ...
    //抓拍机4.0新增
    class NET_ITC_LANE_HVT_PARAM_V50 extends HCNetSDKStructure {
    ...
    class NET_ITC_POST_HVT_PARAM_V50 extends HCNetSDKStructure {
        public byte byLaneNum;    //识别的车道个数，1-6
    ...
    class NET_ITC_LANE_PARAM extends HCNetSDKStructure {
    ...
    //卡口RS485车检器触发参数
    class NET_ITC_POST_RS485_PARAM extends HCNetSDKStructure {
        public byte byRelatedLaneNum;//关联的车道个数
    ...
    //卡口RS485雷达触发参数
    class NET_ITC_POST_RS485_RADAR_PARAM extends HCNetSDKStructure {
        public byte byRelatedLaneNum;//关联的车道个数
        public byte[] byRes1 = new byte[3];
        public NET_ITC_PLATE_RECOG_PARAM struPlateRecog = new NET_ITC_PLATE_RECOG_PARAM(); //牌识参数
        public NET_ITC_LANE_PARAM[] struLane = new NET_ITC_LANE_PARAM[MAX_ITC_LANE_NUM]; //关联的车道参数
    ...
    //单个触发参数结构
    class NET_ITC_SINGLE_TRIGGERCFG extends HCNetSDKStructure {
    ...
    //触发参数结构
    class NET_ITC_TRIGGERCFG extends HCNetSDKStructure {
        public int dwSize;            //结构长度
    ...
    //单个IO接入信号灯参数
    class NET_ITC_SINGLE_IO_LIGHT_PARAM extends HCNetSDKStructure {
    ...
    //IO接入信号灯参数
    class NET_ITC_IO_LIGHT_PARAM extends HCNetSDKStructure {
        public NET_ITC_SINGLE_IO_LIGHT_PARAM[] struIOLight = new NET_ITC_SINGLE_IO_LIGHT_PARAM[MAX_LIGHT_NUM]; //单个IO接入信号灯参数
        public byte[] byRes = new byte[8];
    }

    //单个485接入信号灯参数
    class NET_ITC_SINGLE_RS485_LIGHT_PARAM extends HCNetSDKStructure {
        public byte byLightType; //交通灯导向类型，0-左转灯，1-直行灯，2-右转灯
    ...
    //单组视频检测交通信号灯参数结构
    class NET_ITC_SINGLE_VIDEO_DETECT_LIGHT_PARAM extends HCNetSDKStructure {
    ...
    class NET_ITC_VIDEO_DETECT_LIGHT_PARAM extends HCNetSDKStructure {
    ...
    //交通信号灯参数结构
    class NET_ITC_TRAFFIC_LIGHT_PARAM extends HCNetSDKStructure {
    ...
    //违规检测参数结构
    class NET_ITC_VIOLATION_DETECT_PARAM extends HCNetSDKStructure {
    ...
    //违规检测线参数结构
    class NET_ITC_VIOLATION_DETECT_LINE extends HCNetSDKStructure {
    ...
    //单个车道视频电警触发参数结构
    class NET_ITC_LANE_VIDEO_EPOLICE_PARAM extends HCNetSDKStructure {
    ...
    //视频电警触发参数结构
    class NET_ITC_VIDEO_EPOLICE_PARAM extends HCNetSDKStructure {
        public byte byEnable;    //是否启用，0-不启用，1-启用
    ...
    class NET_DVR_CURTRIGGERMODE extends HCNetSDKStructure {
        public int dwSize;
        public int dwTriggerType; //触发类型，详见ITC_TRIGGERMODE_TYPE
        public byte[] byRes = new byte[24];
    }

    class NET_ITC_VIDEO_TRIGGER_COND extends HCNetSDKStructure {
        public int dwSize;
        public int dwChannel;
    ...
    class NET_ITC_VIDEO_TRIGGER_PARAM extends HCNetSDKStructure {
        public int dwSize;
    ...
    class NET_DVR_CMS_PARAM extends HCNetSDKStructure {
        public int dwSize;
    ...
    //设置完全获取出厂值
    class NET_DVR_COMPLETE_RESTORE_INFO extends HCNetSDKStructure {
        public int dwSize; //结构体长度
        public int dwChannel; //通道号
        public byte[] byRes = new byte[64];
    }

    class NET_DVR_STD_ABILITY extends HCNetSDKStructure {
    ...
    class NET_DVR_STD_CONFIG extends HCNetSDKStructure {
    ...
    class NET_DVR_LOCAL_SDK_PATH extends HCNetSDKStructure {
        public byte[] sPath = new byte[NET_SDK_MAX_FILE_PATH];//组件库地址
        public byte[] byRes = new byte[128];
    }

    class BYTE_ARRAY extends HCNetSDKStructure {
        public byte[] byValue;
    ...
    class INT_ARRAY extends HCNetSDKStructure {
        public int[] intValue;
    ...
    class INTRef_ARRAY extends HCNetSDKStructure {
    ...
    class NET_DVR_JSON_DATA_CFG extends HCNetSDKStructure {
        public int dwSize;                        //结构体大小
    ...
    class CallBack_USER extends HCNetSDKStructure {
    ...
    class NET_DVR_CAMERAPARAMCFG_EX extends HCNetSDKStructure {
        public int dwSize;
    ...
    */
    ...
    class NET_DVR_VIDEOEFFECT extends HCNetSDKStructure {
    ...
    class NET_DVR_GAIN extends HCNetSDKStructure {
    ...
    class NET_DVR_WHITEBALANCE extends HCNetSDKStructure {
    ...
    class NET_DVR_EXPOSURE extends HCNetSDKStructure {
    ...
    class NET_DVR_GAMMACORRECT extends HCNetSDKStructure {
    ...
    class NET_DVR_WDR extends HCNetSDKStructure {
    ...
    class NET_DVR_DAYNIGHT extends HCNetSDKStructure {
    ...
    class NET_DVR_BACKLIGHT extends HCNetSDKStructure {
    ...
    class NET_DVR_NOISEREMOVE extends HCNetSDKStructure {
    ...
    class NET_DVR_CMOSMODECFG extends HCNetSDKStructure {
    ...
    class NET_DVR_DEFOGCFG extends HCNetSDKStructure {
        public byte byMode; //模式，0-不启用，1-自动模式，2-常开模式
        public byte byLevel; //等级，0-100
        public byte[] byRes = new byte[6];
    }

    class NET_DVR_CMOSMODCFG extends HCNetSDKStructure {
        public byte byCaptureMod;   //抓拍模式：0-抓拍模式1；1-抓拍模式2
        public byte byBrightnessGate;//亮度阈值
        public byte byCaptureGain1;   //抓拍增益1,0-100
        public byte byCaptureGain2;   //抓拍增益2,0-100
    ...
    class NET_DVR_ELECTRONICSTABILIZATION extends HCNetSDKStructure {
        public byte byEnable;//使能 0- 不启用，1- 启用
        public byte byLevel; //等级，0-100
        public byte[] byRes = new byte[6];
    }

    class NET_DVR_CORRIDOR_MODE_CCD extends HCNetSDKStructure {
        public byte byEnableCorridorMode; //是否启用走廊模式 0～不启用， 1～启用
        public byte[] byRes = new byte[11];
    }

    class NET_DVR_SMARTIR_PARAM extends HCNetSDKStructure {
    ...
    class NET_DVR_PIRIS_PARAM extends HCNetSDKStructure {
        public byte byMode;//0-自动，1-手动
        public byte byPIrisAperture;//红外光圈大小等级(等级,光圈大小正比例)level:1~100 默认:50（手动模式下增加）
        public byte[] byRes = new byte[6];
    }

    class NET_DVR_LASER_PARAM_CFG extends HCNetSDKStructure {
    ...
    class NET_DVR_FFC_PARAM extends HCNetSDKStructure {
    ...
    class NET_DVR_DDE_PARAM extends HCNetSDKStructure {
    ...
    class NET_DVR_AGC_PARAM extends HCNetSDKStructure {
    ...
    class NET_DVR_SNAP_CAMERAPARAMCFG extends HCNetSDKStructure {
    ...
    class NET_DVR_OPTICAL_DEHAZE extends HCNetSDKStructure {
        public byte byEnable; //0~不启用光学透雾，1~启用光学透雾
        public byte[] byRes = new byte[7];
    }

    class NET_DVR_THERMOMETRY_AGC extends HCNetSDKStructure {
    ...
    class NET_DVR_CHECK_DEV_STATE extends HCNetSDKStructure {
    ...
    class NET_DVR_FLOW_INFO extends HCNetSDKStructure {
        public int dwSize;             //结构大小
    ...
    class NET_DVR_AES_KEY_INFO extends HCNetSDKStructure {
        public byte[] sAESKey = new byte[16];        /*码流加密密钥*/
        public byte[] byRes = new byte[64];          /*保留字节*/
    }

    class NET_DVR_ALARM_RS485CFG extends HCNetSDKStructure {
        public int dwSize;                 // 结构体大小
    ...
    class NET_DVR_ALARMHOST_RS485_SLOT_CFG extends HCNetSDKStructure {
        public int dwSize;              // 结构体大小
        public byte[] sDeviceName = new byte[NAME_LEN];      // 前端设备名称
    ...
    class NET_DVR_VIDEOWALLDISPLAYPOSITION extends HCNetSDKStructure {
        public int dwSize;
        public byte byEnable;
    ...
    int MAX_DISPLAY_NUM = 512; //最大显示输出个数

    class NET_DVR_DISPLAYCFG extends HCNetSDKStructure {
        public int dwSize;
        public NET_DVR_DISPLAYPARAM[] struDisplayParam = new NET_DVR_DISPLAYPARAM[MAX_DISPLAY_NUM];
        public byte[] byRes2 = new byte[128];
    }

    class NET_DVR_DISPLAYPARAM extends HCNetSDKStructure {
        public int dwDisplayNo;
        public byte byDispChanType;
        public byte[] byRes = new byte[11];
    }

    class NET_DVR_WALLOUTPUTPARAM extends HCNetSDKStructure {
        public int dwSize;
    ...
    class WALLOUTPUTPARAM_ARRAY extends HCNetSDKStructure {
    ...
    interface FDrawFun extends Callback {
        void invoke(int lRealHandle, HDC hDc, int dwUser);
    }
    ...
    interface FPlayESCallBack extends Callback {
    ...
    interface DEV_WORK_STATE_CB extends Callback {
        boolean invoke(Pointer pUserdata, int iUserID, NET_DVR_WORKSTATE_V40 lpWorkState);
    }

    interface FLOWTESTCALLBACK extends Callback {
        void invoke(int lFlowHandle, NET_DVR_FLOW_INFO pFlowInfo, Pointer pUser);
    }
    ...
    //gps相关结构定义
    class TimeSegParam extends HCNetSDKStructure {
        //GPS数据查找起始时间
        public NET_DVR_TIME struBeginTime;
        //GPS数据查找结束时间
        public NET_DVR_TIME struEndTime;
    ...
    //按时间点查询
    class TimePointParam extends HCNetSDKStructure {
        //GPS数据查找时间点
    ...
    //gps查询参数定义
    class NET_DVR_GET_GPS_DATA_PARAM extends HCNetSDKStructure {
        ...
        public void read() {
            super.read();
            switch (dwCmdType) {
                case 0 -> gpsDataParam.setType(TimeSegParam.class);
                case 1 -> gpsDataParam.setType(TimePointParam.class);
                default -> {
                }
            }
            gpsDataParam.read();
    ...
    //gps数据结构定义
    class NET_DVR_GPS_INFO extends HCNetSDKStructure {
    ...
    //gps返回数据结构定义
    class NET_DVR_GPS_DATA extends HCNetSDKStructure {
     ...
     */
    //设备抓图附加全屏测温数据结构体
    class NET_DVR_JPEGPICTURE_WITH_APPENDDATA extends HCNetSDKStructure {
        public int dwSize;
        public int dwChannel;//通道号
    ...
    class DATE_TIME extends HCNetSDKStructure {
    ...
    //全屏测温数据解析
    class STREAM_RT_DATA_INFO_S extends HCNetSDKStructure {
    ...
    class STREAM_FS_SUPPLE_INFO_TEMP extends HCNetSDKStructure {
    ...
    class STREAM_FARME_INFO_TEMP extends HCNetSDKStructure {
    ...
    //测温规则温度信息
    class NET_DVR_THERMOMETRYRULE_TEMPERATURE_INFO extends HCNetSDKStructure {
        public float fMaxTemperature;
        public float fMinTemperature;
        public float fAverageTemperature;
        public NET_VCA_POINT struHighestPoint;
    ...
    class REMOTECONFIGSTATUS_THERMOMETRY extends HCNetSDKStructure {
        public byte[] byStatus = new byte[4];
        public byte[] byErrorCode = new byte[4];
    }

    class NET_DVR_FIREDETECTION_ALARM extends HCNetSDKStructure {
        public int dwSize; //结构体大小
        public int dwRelativeTime; //相对时标
        public int dwAbsTime; //绝对时标
        public NET_VCA_DEV_INFO struDevInfo; //前端设备信息
    ...
    //模块服务配置结构体
    class NET_DVR_DEVSERVER_CFG extends HCNetSDKStructure {
        public int dwSize; //结构体大小
    ...
    int MAX_UPLOADFILE_URL_LEN = 240;
    int IMPORT_DATA_TO_FACELIB = 39; //导入人脸数据（人脸图片+图片附件信息 到设备人脸库）

    class NET_DVR_UPLOAD_FILE_RET extends HCNetSDKStructure {

        public byte[] sUrl = new byte[MAX_UPLOADFILE_URL_LEN];   //url
        public byte[] byRes = new byte[260];
    }

    class NET_DVR_FLOW_TEST_PARAM extends HCNetSDKStructure {
        public int dwSize;              //结构大小
    ...
    class NET_DVR_RECORD_TIME_SPAN_INQUIRY extends HCNetSDKStructure {
        public int dwSize;    //结构体大小
        public byte byType;     //0 正常音视频录像, 1图片通道录像, 2ANR通道录像, 3抽帧通道录像
        public byte[] byRes = new byte[63];
    }

    class NET_DVR_RECORD_TIME_SPAN extends HCNetSDKStructure {
        public int dwSize;        //结构体大小
     ...
     * 月历录像分布查询条件结构体
     */
    class NET_DVR_MRD_SEARCH_PARAM extends HCNetSDKStructure {
        public int dwSize;            // 结构体大小
     ...
     * 月历录像分布查询结果结构体
     */
    class NET_DVR_MRD_SEARCH_RESULT extends HCNetSDKStructure {
    ...
    int NET_DVR_GET_GISINFO = 3711;

    //GIS信息
    class NET_DVR_GIS_INFO extends HCNetSDKStructure {
        public int dwSize;
    ...
    //GBT28181协议的设备编码通道配置
    class NET_DVR_GBT28181_CHANINFO_CFG extends HCNetSDKStructure {
        public int dwSize;
    ...
    // 巡航路径配置条件结构体
    class NET_DVR_CRUISEPOINT_COND extends HCNetSDKStructure {
        public int dwSize;
        public int dwChan;
    ...
    // 巡航路径配置结构体
    class NET_DVR_CRUISEPOINT_V40 extends HCNetSDKStructure {
        public int dwSize;
    ...
    // 巡航路径配置结构体
    class NET_DVR_CRUISEPOINT_V50 extends HCNetSDKStructure {
        public int dwSize;
    ...
    // 巡航点参数结构体
    class NET_DVR_CRUISEPOINT_PARAM extends HCNetSDKStructure {
        public short wPresetNo;
        public short wDwell;
        public byte bySpeed;
...
interface PlayCtrl extends Library {
    ...
    class FRAME_INFO extends HCNetSDKStructure {
